...
| `flutter3d_flame` | 42 | | `apps/flutter3d_showcase` | 58 |

The rows sum to 9691 rather than 9715: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
