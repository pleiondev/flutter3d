...
| `flutter3d_flame` | 47 | | `apps/flutter3d_showcase` | 63 |

The rows sum to 9729 rather than 9753: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
