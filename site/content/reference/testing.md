...
| `flutter3d_model_core` | 714 | | `flutter3d_model_mcp` | 67 |
| | | | `flutter3d_net` | 14 |
| | | | `flutter3d_net_webrtc` | 2 |
| | | | `flutter3d_sim_mcp` | 7 |
| | | | `flutter3d_render_mcp` | 4 |

The rows sum to 6651 rather than 6670: the remaining live in `packages/*/example/test`, which the count includes and this table does not — the usual 19 plus whatever a concurrent session's own in-flight work has added there since this snapshot was taken.
