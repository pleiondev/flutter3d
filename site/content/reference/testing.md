...
| `flutter3d_model_core` | 981 | | `flutter3d_model_mcp` | 183 |
| | | | `flutter3d_net` | 14 |
...
The rows sum to 8563 rather than 8587: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
