...
| `flutter3d_mcp_kit` | 2 | | `flutter3d_build` | 57 |
| | | | `apps/flutter3d_lesson_viewer` | 39 |
| | | | `apps/flutter3d_stereo_lesson_viewer` | 6 |

The rows sum to 9265 rather than 9289: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
