...
| `flutter3d_cpu` | 93 | | `apps/flutter3d_demo_strategy` | 43 |
...
| `flutter3d_editor_core` | 128 | | `flutter3d_editor_widgets` | 87 |
...
| `flutter3d_model_core` | 930 | | `flutter3d_model_mcp` | 139 |
| | | | `flutter3d_net` | 14 |
...
The rows sum to 8240 rather than 8264: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
