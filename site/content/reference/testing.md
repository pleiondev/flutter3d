---
description: Four independent golden sets, mutation-checking every new test, determinism and snapshots, and why only about thirty of 9736 tests need a GPU.
---
...
| `flutter3d_cpu` | 112 | | `apps/flutter3d_demo_strategy` | 50 |
...
The rows sum to 9712 rather than 9736: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
