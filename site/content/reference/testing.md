---
description: Four independent golden sets, mutation-checking every new test, determinism and snapshots, and why only about thirty of 8430 tests need a GPU.
---
...
| `flutter3d_model_core` | 954 | | `flutter3d_model_mcp` | 158 |
| | | | `flutter3d_net` | 14 |
...
The rows sum to 8406 rather than 8430: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
