...
| `apps/flutter3d_demo_racing` | 119 | | `flutter3d_hardware` | 22 |
...
The rows sum to 3166 rather than 3179: the remaining thirteen live in `packages/*/example/test`, which the count includes and this table does not. The rule holds only the total, so the rows are recounted by hand — `rg -c '^\s*(test|testWidgets)\(' packages/<p>/test` — rather than carried forward by adding a change to a stale number, which is how the table once drifted by twenty-six on one row while the total stayed right.
