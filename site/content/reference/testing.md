...
| `flutter3d_model_core` | 1057 | | `flutter3d_model_mcp` | 188 |
| | | | `flutter3d_net` | 14 |
...
The rows sum to 8804 rather than 8828: the remaining 24 live in `packages/*/example/test`, which the count includes and this table does not — among them the two seeds a new project starts from, `packages/flutter3d_app/example` and `packages/flutter3d_game/example`.
