...
| `flutter3d_model_core` | 796 | | `flutter3d_model_mcp` | 89 |
| | | | `flutter3d_net` | 14 |
...
The rows sum to 7398 rather than 7417: the remaining live in `packages/*/example/test`, which the count includes and this table does not — the usual 19 plus whatever a concurrent session's own in-flight work has added there since this snapshot was taken.
