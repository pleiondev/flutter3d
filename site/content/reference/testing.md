---
description: Three independent golden sets, mutation-checking every new test, determinism and snapshots, and why only about thirty of 4144 tests need a GPU.
---

# Testing

4144 tests across 27 packages and six applications, counted the same way the `the document says how many tests there are` rule does: a scan of every `test(`/`testWidgets(` call. The rule holds `ARCHITECTURE.md` §13, the README and this page to the answer — the README went on saying 1242 across thirteen packages for as long as nothing compared it with anything. About thirty need a GPU; the [architecture](/core/architecture/) is what keeps the number that low.

| Package | Tests | | Package | Tests |
|---|---|---|---|---|
| `flutter3d` | 928 | | `flutter3d_particles` | 73 |
| `flutter3d_sim` | 491 | | `flutter3d_bridge` | 65 |
| `flutter3d_game_shooter` | 337 | | `pad_input` | 59 |
| `flutter3d_game_racing` | 223 | | `flutter3d_audio` | 55 |
| `flutter3d_game_platformer` | 217 | | `flutter3d_webgl` | 54 |
| `apps/flutter3d_demo_platformer` | 196 | | `flutter3d_hardware` | 54 |
| `flutter3d_cpu` | 187 | | `flutter3d_impeller` | 53 |
| `apps/flutter3d_editor` | 169 | | `apps/flutter3d_demo_strategy` | 42 |
| `apps/flutter3d_demo_racing` | 144 | | `flutter3d_session` | 34 |
| `flutter3d_physics` | 168 | | `pointer_lock` | 28 |
...
The rows sum to 4125 rather than 4144: the remaining 19 live in `packages/*/example/test`, which the count includes and this table does not.
