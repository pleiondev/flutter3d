...
| A web build turns the camera by dragging when the mouse should capture, or shows no on-screen controls on a phone | `Playing` was asking `kIsWeb`, which is one answer to three different questions. It asks the capture backend whether there is a pointer to take and `defaultTargetPlatform` whether the player has fingers — a mobile browser reports itself as `android` or `iOS` |
