...
| The view stops turning when the cursor reaches the edge of the window | Flutter exposes no pointer lock on any desktop platform and does not surface the browser's. `packages/pointer_lock` supplies it on macOS by turning off the association between the physical mouse and the on-screen cursor, and in a browser through `requestPointerLock` |
| A web build turns the camera by dragging when the mouse should capture, or shows no on-screen controls on a phone | `Playing` was asking `kIsWeb`, which is one answer to three different questions. It asks the capture backend whether there is a pointer to take and `defaultTargetPlatform` whether the player has fingers, since a mobile browser reports itself as `android` or `iOS` |
| A shadow has a straight edge in a scene with no straight edges | A normal offset measured in metres. What it clears is the width of one texel of the shadow map, which grows with distance, so a fixed offset is right at one range and too small at every greater one. The surface then shadows itself out to wherever the map runs out of content, and *that* boundary is the straight line |
...
| Every edge in the frame goes jagged once a glass object is in the scene | Since 0.8.0 a frame with a transmissive draw splits the scene around a copy of itself, and the multisampled targets keep nothing for the second pass, so the frame draws one sample a pixel. `FrameResult.antiAliasing.msaaDeclined` names the reason. The question is asked of the scene before culling, so a pane off screen does it too. Put glass on a layer the view does not draw when it is not needed, turn on the temporal resolve, which is the anti-aliasing that survives a split frame, or put `transparent` in `disabledPasses` to draw in one multisampled pass with the glass reading the environment as it did in 0.7.4 |
| Glass shows a flat colour where it used to show the sky | Glass now reads the scene behind it, and where nothing is drawn behind it that is the clear colour. 0.7.4 showed the environment there. Switch the `sky` on, or give the room a back wall |
| Every glTF model is lighter after upgrading to 0.8.0 | `baseColorFactor` is read as linear, as the specification says; a factor of 0.5 used to draw as 0.21. The file is right and the old picture was wrong, so there is nothing to fix unless a value was hand-tuned against the old reading |
| The sun's shadow is a smear across the whole cascade after upgrading | `ShadowSettings.directionalLightRadius` is in radians since 0.8.0. A radius tuned in the old unit is now an enormous sun, clamped at sixteen texels. The real sun's is about 0.0047 |
| Embers, sparks and splats look dim and leave trails with temporal anti-aliasing on | Nothing that blends writes a velocity, so the resolve reprojects the wall behind the ember and keeps nine tenths of it. Set `TemporalSettings.reactive` above nought, and a contributor of your own overrides `PassContributor.encodeReactive` |
| A single captured frame with temporal anti-aliasing on is aliased and shifted by a fraction of a pixel | The resolve needs history, and the first frame has none. Render a few frames before capturing, or turn the resolve off for the capture |
| A splat cloud draws as speckle | `SplatComposite.hashed` asked for by name with no temporal resolve to average it. Leave the composite on `automatic`, which draws the sorted blend when no resolve runs |
...
| Two runs of the same save diverge | Monster thinking was staggered across steps by `Object.hashCode`, which is an address. It is an ordinal now. Anything using `math.Random` has the same problem, so use `GameRandom`, which has readable state |
...
| An agent looks stuck | Two callers sharing one `Navigation` with different destinations. `update` re-targets every field it holds, so the second one's fields flow to the first one's goal without any error |
...
| The first frame after loading spends its whole budget catching up | Loading blocked the ticker and all of that time is in the accumulator. Call `loop.clock.reset()` after a load, since none of that time happened in the game |
| Temporal anti-aliasing is on and the edges are no smoother | The resolve needs a velocity buffer, the velocity needs the surface buffer, and a device that opens one colour attachment has none. There the engine does not even jitter, since a jittered picture nobody averages shakes. `FrameResult.antiAliasing.temporal` says whether it ran |
| An effect works on the desktop and silently does less in a browser | WebGL2 answers four capabilities narrower than the other backends. `supportsFloat32Filtering` is true only where `OES_texture_float_linear` was granted, so the EVSM shadow filter falls back to the 3×3 kernel and `FrameResult.skipped` lists `shadow moments` as `PassSkip.unsupported`. `supportsIndependentBlend` is true only with `OES_draw_buffers_indexed`, so weighted blended transparency draws the list twice. `hdrOutputFormats` is empty, so `OutputTransform.extendedSrgb` draws the SDR frame. `supportsGpuTimestamps` and `supportsCompute` are false, so `FramePass.gpuMicros` is null. Ask the device, and read `skipped`, before comparing timings across backends |
| A material with a clear coat or transmission draws as plain metal-rough | Only `LightingModel.pbrLayered` reads `Material.extensions`. A material built by hand with `LightingModel.pbr` is drawn without its layers. Pick the model with `LightingModel.pbr.withLayers(layers)`, as the loaders do |
| Occlusion culling is on and `FrameResult.culled` does not move | `OcclusionMode.software` rasterises only meshes marked `MeshNode.occluder`, and nothing is marked by default. `OcclusionMode.hiZ` needs nothing marked but answers "visible" until its first readback arrives, after a camera cut and with more than one view |
| `spatialUpscale` is on and the frame comes back small, or unchanged | It runs only with a `renderScale` below one and the temporal resolve off, since the resolve already rebuilds the full size. `SpatialUpscaleSettings.runsFor(settings)` answers for a frame |
| Adaptive quality picks rows that do not match what the device can do | The committed `QualityTable`s were costed on the software rasteriser, and `QualityTable.measured` is false for every class. The controller corrects each row from what it measures, but the order of the rows by cost is a stand-in until the table is measured on the class's own GPU |
