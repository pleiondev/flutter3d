...
The two projects are not after the same thing. Scene is trying to be a complete, production-grade 3D engine and toolkit for anyone building on Flutter. flutter3d answers a narrower question: whether a HAL, a scene graph and a game layer of this shape can be built by someone with no inside access to Impeller, tested against three complete games of different genres that needed no engine-level code changes between them. The stars, likes and downloads below were pulled from pub.dev and the GitHub API on 2026-09-20; the flutter3d version and publication rows were updated after 0.7.0 went out on 2026-09-21. The flutter3d half of the feature lists was read against the source tree again on 2026-09-20. The Scene half still describes its README as of 2026-09-15, except where noted.
...
| Core package version | 0.7.0 | 0.23.0 |
...
| Multiplayer | Rollback netcode for two peers (`flutter3d_net`, WebSocket or WebRTC transport), on pub.dev since 0.7.0 | [`flutter_scene_net`](https://pub.dev/packages/flutter_scene_net), over `dashwire` |
| Editor with an MCP server | Yes: `flutter3d_editor_mcp`, published to pub.dev at 0.7.0 with the rest of the set | Yes: the Flutter Scene Editor stack, shipped as a desktop app, not on pub.dev, and explicitly "in active development" |
...
- **Temporal anti-aliasing and radial lens distortion.** Scene has TAA and SMAA alongside FXAA, plus a radial distortion pass. flutter3d has neither: its anti-aliasing is FXAA with contrast-adaptive sharpening built into that same pass, and it has no distortion pass. Its post pipeline otherwise runs bloom, screen-space ambient occlusion, contact shadows, screen-space reflections, depth of field, light shafts, chromatic aberration, film grain, a vignette, automatic exposure, colour grading from a strip LUT, and a choice of three tone-mapping curves (Khronos PBR Neutral, ACES, AgX), which is more of this list than the previous version of this page credited it with. Turning on any effect that reads the surface buffer still switches MSAA off for the frame.
- **Native physics through a mature third-party engine.** Rapier or box3d, pluggable, where flutter3d has an implementation scoped to this project alone. flutter3d's physics package covers overlap, sweeps, rays and a character controller, with no continuous collision, constraint solver or joints. Audio is not really a difference between the two, since both wrap the same SoLoud engine; Scene additionally offers FMOD, commercial middleware, as a second backend.
...
- **Three complete games of different genres on one engine, with no genre-specific code shared between them.** A shooter, a platformer and a racing game, each built without editing the engine packages the other two depend on, with the boundary enforced by structural scans (`tool/structure.dart`) and not only by convention. A fourth genre, strategy, is in the workspace too, and its genre package is on pub.dev since 0.7.0.
- **A bidirectional bridge to Flame, the established Flutter 2D game engine.** `flame_flutter3d` composites a Flame layer and a flutter3d layer in one widget, each drawing itself, with transform, ECS (through `flutter3d_sim`'s own actor system), physics, input and camera reconciled between the two, so neither engine drives the other's renderer. A fifth demo, Meteor Yard, uses all five end to end. Scene's README (checked 2026-09-20) does not describe an equivalent.
