...
The two are not aimed at the same claim. Scene is trying to be a complete, production-grade 3D engine and toolkit for anyone building on Flutter. flutter3d exists to answer a narrower question: whether a HAL, a scene graph and a game layer of this shape can be built by someone with no inside access to Impeller, proven against three complete games of different genres that share no engine-level code changes between them. The numbers below were pulled from pub.dev and the GitHub API on 2026-09-15, not written from memory. The flutter3d half of the feature lists was read against the source tree again on 2026-09-19; the Scene half still describes its README as of the 15th.
...
- **Temporal anti-aliasing and lens effects.** Scene has TAA and SMAA alongside FXAA, radial distortion, and colour grading from `.cube` LUTs. flutter3d has FXAA, automatic exposure, depth of field, light shafts, chromatic aberration and a strip LUT, but no TAA or SMAA, and turning on any effect that reads the surface buffer switches MSAA off for the frame.
