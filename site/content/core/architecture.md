...
Two rules in `tool/structure.dart` hold the boundary: `the hardware layer names no graphics API` refuses a `flutter_gpu` import anywhere in the HAL, and `the engine names no backend` refuses one in `flutter3d` — and refuses the *dependency* too, which the import scan alone would miss. The HAL also refuses `dart:ui` and the rest of Flutter outright now: `GraphicsDevice.present`, its one Flutter member, moved to `presentFrame` in `flutter3d_app`, which is where the widget a finished frame becomes is decided per concrete backend.
