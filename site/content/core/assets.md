...
Every one of them (the two text formats, the binary container, and STL) emits the same `ModelDocument`: a list of `ModelSurface`, a list of `SurfaceMaterial`, a list of `EncodedImage`, and `warnings`.
...
| UVs | V is flipped by default, because OBJ texture space has its origin at the bottom left |
...
| Passes | The colour, shadow and id passes all morph, because a shadow of the base shape is a bug that hides |
