...
<div class="note">
<p>The backend choice was deliberately not folded into <code>flutter3d_session</code> — it went to <code>flutter3d_app</code> instead, once <code>flutter3d_backend</code> merged there. Session would then depend on both backends, and <code>apps/flutter3d_editor</code>, which is desktop only, has no browser build to choose for, and depends on <code>flutter3d_session</code> directly rather than through the <code>flutter3d_app</code> barrel, would pull WebGL through it for nothing.</p>
</div>
...
The packages this page walks are behind `flutter3d_app`, so an application names the layer once rather than naming its parts:
