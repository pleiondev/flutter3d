...
The repository is a [pub workspace](https://dart.dev/tools/pub/workspaces): one resolve covers all twenty-seven packages and six applications against a single lock file. Packages that depend on each other by path drift apart at the first version bump otherwise, and the drift only shows up as an unbuildable checkout on somebody else's machine.
...
4104 tests across 27 packages and six applications, and only about thirty need a GPU: the Impeller half of the golden set. The other half renders through the software backend, so forty-three scenes stay checkable in a headless run.
