...
**https://flutter3d.pleion.dev/**. Public since 2026-08-29; it sat behind
basic auth while the repository was private.
...
| Headers | COOP/COEP site-wide (SharedArrayBuffer for the demos' audio and skwasm threads) and a `.mjs → text/javascript` type override — both in the vhost, and both load-bearing: without the first every demo plays silent, without the second every demo is a blank page |
...
the vhost and the tunnel stay put.

The basic auth came off on 2026-08-29, ahead of publishing the packages. The
htpasswd file and a pre-removal copy of the vhost
(`flutter3d.pleion.dev.bak-basicauth`) are still on the server should it need
to go back up; putting it back is restoring that file and reloading nginx.
