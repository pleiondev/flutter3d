...
void main() {
  ...
  test('a level with no textures builds with nothing to report', () async {
    final loaded = await const LevelLoader()
        .build(_level(), device: device, registry: registry);
  ...
  test('and a texture that will not load is a warning, not a console line',
      () async {
...
ByteData _onePixelPng() => ByteData.sublistView(Uint8List.fromList(<int>[
