...
void main() {
  ...
  test(
    'a real run writes the hook, both pubspec entries, and .gitignore',
    () async {
      writePubspec(_freshPubspec);
  ...
  test('a second run is a clean no-op', () async {
    ...
    final hookBefore = File(
      '${scratch.path}/hook/build.dart',
    ).readAsStringSync();
...
''');
  ...
  test('a source in a subdirectory gets its own assets: entry too — '
      'Flutter does not bundle a declared directory recursively', () async {
    ...
    final steps = planInit(scratch);
    expect(
      steps.any((s) => s.description.contains('flutter3d_generated/models/')),
      isTrue,
      reason:
...
''');
  ...
  test('the constraint init writes admits the version this package is', () {
    // The constant is a second copy of a number `pubspec.yaml` owns, and a
    // release that bumps one without the other sends every project `init`
  ...
  test('a missing target directory fails cleanly', () async {
    final err = _BufferSink();
    final code = await runInit(<String>[
      Directory('${scratch.path}/nope').path,
    ], err: err);
    expect(code, 1);
