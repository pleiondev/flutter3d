...
///
/// `test/fixtures/ktx2/etc1s_gradient_quadrants.ktx2` was produced by a
/// from-source build of `BinomialLLC/basis_universal`'s `basisu` CLI (not
/// vendored here — `toktx`/`basisu` are not available as packages):
///
/// ```
...
///
/// The three `.ktx2` files here are copies of
/// `flutter3d_samples/assets/ktx2/*` — this package cannot depend on
/// `flutter3d_samples`, which needs the Flutter SDK for its own
/// `flutter: assets:` declaration, so the bytes this suite reads are kept
/// alongside it instead. Runs off-device: reading the file is the only I/O,
/// same as the samples package's own tests.
library;
...
Uint8List _readFixture(String name) =>
    File('test/formats/fixtures/ktx2/$name').readAsBytesSync();
...
void main() {
  test(
    ...
    () {
      final bytes = _readFixture('etc1s_gradient_quadrants.ktx2');
  ...
  for (final (name, width, height, levels) in <(String, int, int, int)>[
    ...
    test(
      ...
      () {
        final texture = Ktx2Texture.parse(_readFixture('$name.ktx2'));
        ...
        for (var level = 0; level < levels; level++) {
          final expected = File(
            'test/formats/fixtures/ktx2/${name}_level_$level.rgba',
          ).readAsBytesSync();
