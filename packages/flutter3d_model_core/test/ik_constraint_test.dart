...
ModelObject _joint(int id, {int? parent, required Vector3 localOffset}) => ModelObject(
  id: id,
  name: 'joint$id',
...
void main() {
  group('resolveIkConstraint', () {
    test('anim-15\'s own acceptance: the effector reaches a target within reach', () {
      ...
      final solved = resolveIkConstraint(project: project, constraint: constraint);
      final effector = _effectorPosition(project, constraint, solved.root, solved.mid);
    ...
    test('a target farther than the chain can reach still gives a finite, fully-extended answer', () {
      final project = _straightChain();
      final constraint = IkConstraint(
        rootJointId: 1,
        midJointId: 2,
    ...
    test(
      'a chain resting bent (not straight) still reaches a nearby target, '
      ...
      () {
        ...
        final effector = _effectorPosition(bent, constraint, solved.root, solved.mid);
    ...
    test(
      'a chain already bent keeps bending the same way, rather than folding '
  ...
  group('bakeIk', () {
    test("anim-15's own acceptance, baked: the effector stays within 1e-3 across the clip", () {
      final project = _straightChain();
      final constraint = IkConstraint(
        rootJointId: 1,
        midJointId: 2,
      ...
      final baked = bakeIk(project: project, clip: clip, constraint: constraint, fps: 30);
      ...
      for (final time in <double>[0.0, 0.25, 0.5, 0.75, 1.0]) {
        ...
        final root = Quaternion(rootOut[0], rootOut[1], rootOut[2], rootOut[3])..normalize();
        final mid = Quaternion(midOut[0], midOut[1], midOut[2], midOut[3])..normalize();
    ...
    test('tracks for joints outside the constraint survive baking untouched', () {
