...
void main() {
  group('anim-17\'s own acceptance', () {
    test('the same skeleton — identity', () {
      ...
      final elbowRot = Quaternion.axisAngle(Vector3(1, 0, 0), 0.7)
        ..normalize();
    ...
    test(
      'height x2, mismatched leg proportion — the foot lands within 1cm '
      ...
      () {
        ...
        double ankleY(ProjectClip retargeted) {
          ...
          final kneeRot = rotOf(
            target.idOf['leftKnee']!,
            Quaternion.identity(),
          );

          final hipWorldPos =
              hipsWorld() + Quaternion.identity().rotated(
                hipRest.getTranslation(),
              );
    ...
    test(
      'rest-relative math holds at a non-identity rest pose '
      ...
      () {
        ...
        final animated = Quaternion.axisAngle(Vector3(0, 0, 1), 1.1)
          ..normalize();
        final clip = ProjectClip(
