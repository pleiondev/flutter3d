...
public class PointerLockPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  private static let methodChannelName = "dev.flutter3d/pointer_lock"
  private static let eventChannelName = "dev.flutter3d/pointer_lock/events"
  ...
  private func capture() {
    ...
    captured = true
    emit(state: PointerLockPlugin.stateCaptured)
  }
  ...
  private func release(notify: Bool) {
    ...
    if notify && wasCaptured {
      emit(state: PointerLockPlugin.stateReleased)
    }
  }
