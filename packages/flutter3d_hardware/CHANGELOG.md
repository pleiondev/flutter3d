...
* **`bindUniformBlock`'s contract says which of the two failures is an error.**
  A block the compiler dropped is false; a block missing a member the caller
  named throws. The doc comment used to promise the opposite for members, with
  a reason that did not hold, and the two hardware backends had drifted apart
  under it. A backend with no reflection of its shaders is not asked to tell
  the two apart — the software rasteriser hands the block to a Dart shader that
  looks up what it needs by name — and the conformance suite asks a backend
  which kind it is rather than assuming.
