...
const Map<ParityScene, List<int>> kImpellerGrids = <ParityScene, List<int>>{
  ...
  // **Not Impeller's, unlike every other entry here, and it says so.** The
  // recorded grid predated `62b1dd8b` (18 September), which moved film grain
  // after the sRGB encode. Additive noise on black can only lift it, so the
  // 188 black cells went from 0 to 2 on every backend at once, and this entry
  // read as WebGL disagreeing with Impeller across the whole frame: a mean of
  // 1.87 against a budget of 1. That commit said the table needed
  // re-recording; nobody did it, and the check stayed red.
  //
  // Recorded on 2026-09-23 from the software backend, because the machine
  // that did the work has no Xcode and `parity_main.dart` cannot build for
  // macOS without it. The comparison keeps its point: the software backend
  // shares no shader code with WebGL, and the two agreed to a mean of 0.16
  // and a worst cell of 2, where both stood 1.8 above the old entry. Re-record
  // it from Impeller with the rest of the table when that is next done.
