...
/// cannot: is `flutter3d_hardware` *implementable*, or only callable? Everything
...
class _HarnessAppState extends State<HarnessApp> {
  ...
  void _run() {
    ...
    _check(
      device.shaders['NoSuchStage'] == null,
    ...
    _check(
      program.vertexFloats == 5,
      'reconstructed a 5-float vertex (3 position + 2 texcoord)',
    );
    _check(
      program.blocks.containsKey('FragInfo'),
      'reflected the FragInfo block',
    );
    _check(
      program.blocks['FragInfo']!.offsets.containsKey('base_color'),
      'reflected base_color by member name',
    );
    _check(
      program.samplers.containsKey('base_color_texture'),
      'reflected the sampler slot',
    );

    // --- resources ---------------------------------------------------------
    final vertices = device.uploadGeometry(
      ByteData.sublistView(kVertices),
      GeometryUsage.vertices,
    );
    final indices = device.uploadGeometry(
      ByteData.sublistView(kIndices),
    ...
    _say(
      device.debugDrainErrors('after beginRenderPass') ??
          'no gl error after beginRenderPass',
    );
    ...
    _say(
      device.debugDrainErrors('after bindVertexBuffer') ??
          'no gl error after vertices',
    );
    pass.bindIndexBuffer(indices, IndexType.int32, 3);
    _say(
      device.debugDrainErrors('after bindIndexBuffer') ??
          'no gl error after indices',
    );

    final bound = pass.bindUniformBlock(
      vertex,
      'FragInfo',
      <String, Float32List>{
