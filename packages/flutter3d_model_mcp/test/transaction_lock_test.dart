...
void main() {
  group("mcp-14n's own lock, over a command tool", () {
    test(
      'a command called mid-drag does not land until the drag ends',
      () async {
    ...
    test(
      'a command called with nothing open runs immediately, same as always',
      () async {
        final session = sessionWithOneBlock();
    ...
    test(
      'the queued command becomes its own step, not part of the drag it waited on',
      () async {
        final session = sessionWithOneBlock();
        session.history.beginTransaction();
        ...
        // produced.
        expect(session.history.journal.map((c) => c.name), <String>[
          'moveBy',
          'rename',
        ]);
      },
