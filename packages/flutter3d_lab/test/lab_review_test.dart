...
void main() {
  test('null when the student never touched the assigned parameter', () {
    ...
    expect(firstLabDivergence(assignment, student, path: 'length'), isNull);
  });
  ...
  test(
    'a student who overshoots and returns still diverges at the first step',
    () {
      final assignment = _traceOf(<double>[1.2, 1.2, 1.2, 1.2]);
      final student = _traceOf(<double>[1.2, 0.8, 1.2, 1.2]);

      final divergence = firstLabDivergence(
        assignment,
        student,
        path: 'length',
      );
  ...
  test(
    'a step the assignment never recorded is skipped, not a false positive',
    () {
