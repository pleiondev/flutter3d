...
The models `flutter3d` is tested against and its demo browses. This package is
test data rather than a library: the whole of the Dart here is two path
constants.
...
loads, and the engine's published archive was mostly somebody else's data. The
decoder tests still read every path above and the demo still browses them, so
the files moved instead of being deleted. A game depends on the engine and gets
no fixtures; a test or a demo depends on this package.

## Licence

The package is MIT, like the rest of this repository. The models are not ours
and keep their own terms, recorded per file with author and source in
