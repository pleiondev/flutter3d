...
void main() {
  ...
  group('a 2x2 grid', () {
    test('each tile maps its own quarter of NDC space to the whole cube', () {
      ...
      const depth = -2.0;
      final centre = base.projectToNdc(
        Vector3(0, 0, depth),
        aspect: aspect,
      );
      ...
      for (final (tileX, tileY, wantX, wantY) in <(int, int, double, double)>[
        ...
        expect(
          ndc.x,
          closeTo(wantX, 1e-9),
          reason: 'tile ($tileX, $tileY) x',
        );
        expect(
          ndc.y,
          closeTo(wantY, 1e-9),
          reason: 'tile ($tileX, $tileY) y',
        );
      }
    });
  ...
  group('delegation to base', () {
    test('near, far and field of view all come from base', () {
      ...
      final tile = TiledProjection(wide, tileX: 0, tileY: 0, tilesX: 3, tilesY: 2);
  ...
  group('with no tiling at all', () {
    test('a 1x1 grid is the same projection as base', () {
      final tile = TiledProjection(base, tileX: 0, tileY: 0, tilesX: 1, tilesY: 1);
      final baseNdc = base.projectToNdc(Vector3(0.3, -0.2, -3.0), aspect: aspect);
      final tileNdc = tile.projectToNdc(Vector3(0.3, -0.2, -3.0), aspect: aspect);
