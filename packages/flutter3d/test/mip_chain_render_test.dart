...
Future<Uint8List> _renderPatch(
  ...
  final handle = await uploadEncodedImage(
    device,
    encode(checkerboard),
    // `defaultImageDecoder` became a required argument when the upload path
    // stopped assuming `dart:ui` was reachable: this test runs under
    // `flutter_test`, so it is.
...
double _meanRed(Uint8List patch) {
  final reds = <double>[
    for (var i = 0; i < patch.length; i += 4) patch[i].toDouble(),
  ];
  return reds.reduce((a, b) => a + b) / reds.length;
}

void main() {
  test('a gamma-correct chain reads brighter at distance than the engine\'s '
      'own linear-space one, on the same bytes', () async {
