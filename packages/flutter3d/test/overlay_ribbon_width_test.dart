...
void main() {
  test(
    'a ribbon of the same width reads the same pixels wide at two depths',
    () async {
      ...
      expect(
        ...
        reason: 'the near ribbon did not draw',
      );
      expect(
        greenColumn,
        greaterThanOrEqualTo(0),
        reason: 'the far ribbon did not draw',
      );

      final nearWidth = _spanWidth(
        rgba,
        _height ~/ 2,
        redColumn + 1,
        r: 255,
      ...
      expect(
        nearWidth,
        greaterThan(2),
        reason: 'too thin a span to be measuring anything',
      );
    },
  );
}
