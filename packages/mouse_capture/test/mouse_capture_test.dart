...
final class FakeMouseCapturePlatform extends MouseCapturePlatform {
  FakeMouseCapturePlatform({this.supported = true});
  ...
  Future<void> capture() async {
    ...
    // MouseCapture.capture() await this future, so let it land first.
...
void main() {
  late FakeMouseCapturePlatform platform;
  late MouseCapture capture;

  setUp(() {
    platform = FakeMouseCapturePlatform();
    capture = MouseCapture(platform: platform);
  });
  ...
  group('unsupported platforms', () {
    late FakeMouseCapturePlatform unsupported;
    late MouseCapture unsupportedCapture;

    setUp(() {
      unsupported = FakeMouseCapturePlatform(supported: false);
      unsupportedCapture = MouseCapture(platform: unsupported);
    });
