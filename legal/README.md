...
**English only, deliberately.** The application's interface is translated into
Russian and will be translated further; these documents are not. A translated
legal text is a second legal text — it can drift from the original, and when
the two disagree a reader has no way of knowing which one binds. One authentic
version, in the language the licence, the repository and the rest of the
documentation are already written in, is the honest arrangement. The
application shows these texts in English whatever its interface language is
set to, and says so where it shows them.

**These are the documents, not the advice.** They were drafted against what the
software actually does — checked against the source rather than copied from a
template — and they have not been reviewed by a lawyer. Before the first store
submission, have them read by one: the sections most worth an hour of somebody
...
- [ ] **`privacy@pleion.dev` and `legal@pleion.dev` have to exist** and reach a
      mailbox somebody reads. Every document names them; a GDPR request sent to
      an address that bounces is a breach of Article 12(3) on its own.
- [ ] **Name the hosting provider** in section 3 of the privacy policy, and
      have its data processing agreement on file (Article 28 GDPR). The text
      currently says the identity "is stated on the published version of this
      page" — that sentence is a placeholder for a real name.
- [ ] **Confirm the log retention** the provider actually applies. The policy
      promises "in no case longer than 30 days"; if the provider keeps them
...
- [ ] **Publish the pages before the first store submission.** Both stores
      check that the privacy policy URL resolves.

## The store questionnaire answers

Kept here so they can be checked against the privacy policy rather than
remembered, and so the next submission does not re-derive them.
...
**Apple — export compliance.** "Does your app use encryption?" → **No.** The
basis is in `export-compliance.md` § 2.
...
where a member state has not lowered it); not designed for children, so the
Families policy and its SDK requirements do not apply.
...
changed in the file. The repository's history is the change log the documents
themselves point at, so a rewrite that loses that history loses the promise.
