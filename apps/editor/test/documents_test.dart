...
/// `--dart-define=level=../dungeon/assets/levels/crypt.json`, which is exactly
/// right relative to `apps/editor`, and it said `PathNotFoundException: Cannot
...
import 'package:editor/src/documents.dart';
...
void main() {
  const executable =
      '/repo/apps/editor/build/macos/Build/Products/Debug/editor.app'
  ...
  test('a path relative to the application is found from the bundle', () {
    ...
    final found = Documents.find(
      '../dungeon/assets/levels/crypt.json',
      from: roots,
      exists: _disk(<String>{'/repo/apps/dungeon/assets/levels/crypt.json'}),
    );

    expect(found, '/repo/apps/dungeon/assets/levels/crypt.json');
  });
  ...
  test('and so is the same file spelled from the repository root', () {
    ...
    final found = Documents.find(
      'apps/dungeon/assets/levels/crypt.json',
      from: roots,
      exists: _disk(<String>{'/repo/apps/dungeon/assets/levels/crypt.json'}),
    );
  ...
  test('and the working directory wins over the bundle', () {
    ...
    final found = Documents.find(
      ...
      from: roots,
      exists: _disk(<String>{
        '/elsewhere/apps/dungeon/assets/levels/crypt.json',
        '/repo/apps/dungeon/assets/levels/crypt.json',
      }),
    );

    expect(found, '/elsewhere/apps/dungeon/assets/levels/crypt.json');
  });
  ...
  test('and what it could not find says where it looked', () {
    ...
    final said = Documents.couldNotFind('../dungeon/x.json', tried);

    expect(said, contains('Looked in:'));
    expect(said, contains('/dungeon/x.json'));
    expect(said.split('\n').length, lessThan(12),
  ...
  group('the paths it builds', () {
    test('are resolved, so what it prints is what it means', () {
      final built = Documents.candidates(
        '../dungeon/assets/levels/crypt.json',
        from: <String>['/repo/apps/editor'],
      );

      expect(built.single, '/repo/apps/dungeon/assets/levels/crypt.json');
    });
