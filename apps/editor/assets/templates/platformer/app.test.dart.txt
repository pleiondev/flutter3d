...
/// `apps/editor/lib/src/scaffold.dart`. A test that only ever exists as text
...
///
/// It is also `apps/template_app`'s only test, and until now this application
...
void main() {
  ...
  test('and every model it names is a file that is really there', () {
    ...
    // **Skipped in `apps/template_app` and not in a project made from it**,
