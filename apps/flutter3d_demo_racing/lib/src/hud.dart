...
class RaceHud extends StatelessWidget {
  const RaceHud({super.key, required this.readout, this.issue});

  final RaceReadout readout;

  /// Something that went wrong where the player can see it, or null.
  ///
  /// **This game had nowhere to say anything.** Its `SettingsFile` took the
  /// default sink, which prints to a console no player has, so a settings
  /// document that would not read reset every binding in silence — and a
  /// silent reset is indistinguishable from having changed nothing.
  final String? issue;
              ...
              child: HudPanel(
                children: <Widget>[
                  if (readout.mode != RaceMode.freeRoam) ...<Widget>[
                    ...
                    HudLine(
                      label: 'TIME',
                      value: formatLapTime(readout.lapTime),
                    ),
                    HudLine(
                      label: 'BEST',
