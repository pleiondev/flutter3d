...
///
/// It is also `packages/flutter3d_game/example`'s own test — the one
/// application in the repository whose whole job is to be copied.
///
...
void main() {
  ...
  test(
    ...
    () {
      ...
      // **Skipped in `packages/flutter3d_game/example` and not in a project made from it**,
