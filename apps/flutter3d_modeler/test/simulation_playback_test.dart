...
SimulationCacheStub _bakeFallingCloth() {
  final mesh = ClothMesh.grid(cols: 6, rows: 6, spacing: 0.1, height: 1.0, mass: 0.05);
  const settings = ClothSettings();
  const dt = 1 / 60;
  final floor = ClothObstacle(CollisionBox(Vector3(5, 0.05, 5)), Vector3(0.25, 0.2, 0.25));
...
void main() {
  group("pro-sim-04's own acceptance", () {
    ...
    test('playSimulationFrame writes frame 0, then frame 60, onto the mesh', () {
      final cache = _bakeFallingCloth();
      ...
      final deviceMesh = DeviceMesh.upload(device, meshData, keepSourceData: false);
    ...
    test('draws a wireframe box around the collision obstacle the cloth fell onto', () {
      final floor = ClothObstacle(CollisionBox(Vector3(5, 0.05, 5)), Vector3(0.25, 0.2, 0.25));
