...
flutter3d, this repository's own. Targets are the web, the desktop, a tablet
with a pen, and a phone. The interface is a minimal reading of Material 3.

The document was written in Russian and translated here. The interface labels
quoted below stay in the original, with English beside them, because those are
the strings the mock-ups actually show. The application ships both languages
...
`SegmentedButton`, `Slider`, `BottomSheet` and `FilledButton` components). The
HTML itself is not carried across. Every size and colour below is in Flutter
logical pixels, where 1 CSS px is 1 lp.

To open the mock-ups, load `designs/Экраны редактора.dc.html` in a browser. The
second file, `designs/План разработки 3D-редактора.dc.html`, is the
development plan: phases, risks, architecture.

## Fidelity

High. The colours, sizes, density and composition of the panels are final and
are to be reproduced exactly. The exception is what is inside the viewport
(models, skeletons, cloth), which is schematic filler; flutter3d draws the real
geometry.

The page around the mock-ups (the serif face, the light background, the
captions) is the document's own wrapper and is not part of the product. The
application is the dark interface inside the rounded rectangles.

## The Material 3 theme

A dark scheme, with one accent across the whole editor.
...
| Properties panel width | 250 to 330, typically 272 or 290 |
| Status line height | 30 |
| Window radius | 12 |
| Switch segment | height 30, radius 14, container radius 16, padding 2 |
| Pill button | padding 8 × 16, radius 16 |
| List row | padding 6 to 7 × 8, radius 8 |
| Numeric field | padding 6 × 8 to 10, radius 6 |
...
bar, 16 to 18 in lists and fields, 22 to 24 on touch targets.

### Density

M3's compact density: a list row is 32, a rail button 36. The 48 touch target
is kept only on touch platforms (tablet and phone), which are also where the
side panels give way to a floating palette and a sheet.

## The frame

Every mode uses the same frame; only its contents change.
...
4. The status line is permanent: counts on the left, export readiness on the
   right, green `#7EE081` for fine and orange `#FFB86B` for a warning.
...
| Tablet (600 to 1199) | the viewport fills the screen; a floating palette on the left with 48 buttons; the mode switch centred at the top; properties in a sheet about 200 tall |
...
«Смещение» (offset) slider with a numeric field, checkboxes. Below it is a
summary of the selection: faces, perimeter, area.
This screen exists for one requirement: a value set with the mouse is
...
### 05 · Material (phase 2)
A viewport previewing the material, and under it a collapsible node graph 250
tall: nodes 170 to 210 wide at radius 10, a node's header in
...
There are two levels of access: sliders for the simple case, and the graph,
collapsed by default, for the hard one.
...
An island's colour shows its stretch: `primary` for fine, `#FF458E` for
stretched.
...
centre.
The layout is the same on desktop and tablet, because it is drawn for a pen.
...
summary with vertices, triangles and height.
The profile stays editable after the shape is created, because the shape is
kept parametrically.

### 10 · Retopology and baking (phase 4)
The source mesh shows through the new one, and a floating slider at the bottom
left controls how much. The new mesh is `primary` curves at 1.6; the active
quad is `#FF458E` at 22% fill with a 2 outline and ⌀9 corners.
The right panel is 290 wide, in two blocks split by a line. The first is
automatic retopology: the target quad count, checkboxes, and a «Пересобрать
сетку» (rebuild mesh) button on `surfaceContainerHigh`. The second is map
baking: the source-to-target pair chosen automatically, the maps with
checkboxes and a resolution, the cage thickness, and a primary «Запечь N
карты» (bake N maps) button.
...
and the accuracy.
Under the viewport is a strip 150 tall: the transport, a «Запечь симуляцию»
(bake simulation) button, and the cache bar, a 16 lp track at radius 8 with
the solved frames filled `#004F58` and the current frame `#FF458E`.
...
Editing a node recomputes only the branch it affects, and the status line
...
at the top right.
Under the viewport is a 74 strip with a bend slider for the selected bone
...
34 × 34 icon, a name and a duration. In the middle, two viewports side by
side: the source (a grey mocap skeleton) and the target (the character with its
...
(map automatically) link, a root motion block (`В анимации` / `Кодом`, meaning
in the animation or in code) and the corrections (fit to height, plant the
feet).

### 15 · Morphs and facial animation (phase 3)
A 330 right panel lists the shapes. Each row has a name (96), a slider, a value
and a key dot on the right: `radio_button_checked` `#FF458E` where a key
exists, `radio_button_unchecked` `#3F484A` where it does not. A key is set in
the row itself; there is no separate editor.
Below that are corrective shapes bound to a driving bone's angle, and shape
sets as chips.
In the viewport, the shape's points are `primary` for the ordinary ones and
...
(initial weights, symmetry), and a summary card showing how many bones will be
...
the geometry visibly simplifies left to right. Under them is a 96 strip with a
camera distance slider whose track is split into three zones (`#004F58`,
`primary`, `#2C3A3D`), with the current distance marked `#FF458E`.
...
checkboxes for silhouette, UV borders and weight transfer.
...
overlay at the top left on `rgba(11,14,15,0.72)` shows frames a second, draw
calls, triangles in the frame, and bones.
A floating control panel at the bottom centre: pause, clip segments, looping.
A 330 right panel holds the profile's budgets as four rows with a 5 lp fill bar
...
This is the last check before export, and it uses the same runtime the game
runs.
...
- **A mouse operation becomes an editable number.** Any interactive operation
  ...
  nothing to confirm.
- **History.** Every edit goes through a command. Undo, redo and autosave are
  built on commands, and collaborative editing would be built on them later.
...
- **Export checks are continuous**: the status line is recomputed after every
  edit. Orange warns and does not block.
...
- `playback`: the current frame, the range, whether it is playing, the active
  clip, the blend.
- `projectProfile`: the target engine and its budgets, which every check
  reads.
