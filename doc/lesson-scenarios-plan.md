...
| ls-i-01 | **A product configurator.** The non-game `tpl-04` template ("viewer, configurator, device twin") with a real product: a step's annotation changes colour or material and shows price/specification alongside. Built, under `tpl-04`'s own honest boundary (tooling-plan.md §"tpl-04"): `configurator.json`'s `configurator-panel` annotation cycles three named variants and their price live, on tap — the "buyer changes a variant through an annotation... and sees the specification update" half of the acceptance is literally true today. The geometry half closed 2026-09-14, by a different route than first assumed: not live brush-addressing (`Brush` still carries no name, and its material is still baked in at load — that route stays closed), but three pre-built `part` entities the level itself places — `product-red`/`product-blue`/`product-green` — swapped by `SceneNode.visible` the same mechanism `flutter3d_lesson_viewer`'s own teardown already proves, named after `ConfiguratorController.options` by convention (`main.dart`'s own `_applyProductVariant`). Checked directly: the three parts load as real, distinctly coloured nodes, and a drift test fails the day an option is added with no matching part (`apps/flutter3d_template_app/test/tpl04_levels_test.dart`). The honest boundary shrinks rather than closes: a variant still has to be authored as its own geometry ahead of time, not retinted from an arbitrary player-made brush live — a real product line's fixed set of finishes fits this, a sculpted one-off would not | steps, annotations, property binding | tpl-04, wg-01 | S | a buyer changes a variant through an annotation rather than a separate menu, and sees the specification update |
