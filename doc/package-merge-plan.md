...
### 3.5 `flutter3d_sim_mcp` + `flutter3d_render_mcp` → `flutter3d_sim_mcp` — сделано 2026-09-15
...
**Проверено перед переносом, а не предположено: ни у одного пакета нет
`bin/`.** Оба сервера запускаются только своими же тестовыми фикстурами
(`test/fixtures/*_server.dart` внутри `flutter test`); ни один агент нигде в
репозитории не называет процесс по буквальной команде запуска. «Оба `bin/`
сохраняются, пока есть конфигурации агентов, которые их называют» из
исходной формулировки этого пункта — сохранять было нечего, и называть было
некому.

**Настоящая находка: `sim_mcp` уже наполовину содержал `render_mcp`.**
`sim_renderer.dart`'s `SimRenderer` уже оборачивал `DiagnosticRenderer` из
`render_mcp` для собственного `frame` — «два раза написанный один класс», по
собственному докстрингу `SimRenderer`. У `flutter3d_render_mcp` во всём
репозитории был ровно один внешний потребитель, тот самый файл; слияние —
перенос трёх файлов (`diagnostic_renderer.dart`, `diagnostic_session.dart`,
`diagnostic_tools.dart`) в `sim_mcp/lib/src/`, а не перестройка графа
зависимостей.

**Единственное реальное препятствие — совпадение имён инструментов.** У
обоих серверов был `open`, у обоих — `frame`; агенту с одной таблицей
инструментов нельзя предложить две с одним и тем же именем. Пять
инструментов `render_mcp` переименованы с приставкой `diag`
(`diagOpen`/`diagFrame`/`diagPixel`/`diagPasses`/`diagScanNaN`) — не только
два столкнувшихся, ради единообразия. `SimSession.diagnostic` держит
состояние второй половины отдельно от первой: играть уровень вслепую и
разбирать кадр — два разных вопроса с разным состоянием, а не одна сессия
с двумя именами.

`packages/flutter3d_render_mcp` удалён целиком; `git log` пакета `sim_mcp` —
его история с этого момента. Оба интеграционных набора тестов (реальный
сокет, реальный второй процесс) перенесены и проходят: `sim_mcp_test.dart`
(11 инструментов вместо 6 в списке), новый `diagnostic_mcp_test.dart` (те же
два теста `render_mcp_test.dart`, с переименованными инструментами).
`flutter analyze` чисто. Единственный красный тест — «an agent walks the
crypt...», `LevelFormatException` на `widget_surface` в `ShooterHeadlessGame`'s
собственном `registry()` — уже был красным на `HEAD` до этого захода вообще
(проверено в отдельном `git worktree`), не имеет отношения к слиянию, и не
тронут. `dart run tool/structure.dart` — 32/32 после правки числа тестов
...
| `flutter3d_sim_mcp` | ~1500 | нет | §1.3 `dart_mcp` | приняла `render_mcp` (3.5, сделано 2026-09-15) |
