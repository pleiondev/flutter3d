...
| wg-01 ⚙ | **`WidgetSurface`.** A scene node: a live widget, redraw by dirtiness, pointer and focus routing, a keyboard while focused, semantics in §7. In the level document — a reference to a widget by name from the application's own registry. Built and tested in `flutter3d_session` (`widget_surface.dart`, `widget_surface_pipeline.dart`) and `flutter3d_bridge` (`WidgetSurfaceKind`, `widget_surface_visuals.dart`): hit-testing maps a world ray to the surface's own UV, `redrawCount` stays flat across untouched frames and advances only when the widget marks itself dirty, and a tap routes real keyboard focus into a control inside the surface | session, sim, editor | L | wg-00 | a text field on an in-game screen accepts keyboard input; a list scrolls under a finger; a frame with no widget change doesn't redraw it (a counter in the diagnostics) |
...
| tpl-04 ⚠ | **Non-game templates.** A viewer, a configurator, a device twin — over the edu-00 document and `WidgetSurface`. Built: all three as real level documents in `apps/flutter3d_template_app` (`viewer.json`, `configurator.json`, `twin.json`), each with its own tested widget (`template_widgets_test.dart`'s viewer and configurator controller tests; `operator_panel_test.dart` for the twin's dashboard, which is `wg-02`'s own `OperatorPanel` reading `edu-05`'s `SamplerDataSource`). Not done: no `--list` flag in the application, and — the same gap `wg-02`'s own row names — none of the three is embedded in `tpl-02`'s gallery, so today they are reachable only by opening the template application itself. **Removed 2026-09-15 by owner decision** (`docs/package-architecture-review.md` §8): the digital twin, the configurator and the viewer are products, not engine templates — the digital-twin model lives in the owner's own separate project — so the three levels, their widgets and tests, and `flutter3d_twin` left the repository, and the template application itself became two package examples (`packages/flutter3d_app/example`, `packages/flutter3d_game/example`). `edu-05`'s data sources stay in `flutter3d_sim`, where `flutter3d_lab` uses them | build, apps | M | wg-01, edu-01 | three templates in `--list` and in the gallery |
...
| edu-02 ⚠ | **A public page and an embed.** An interactive's own page, an iframe with parameters, a viewer as a web build on the engine. Built: `cloud/lessons/server` — `flutter3d_lessons`, serving `lessons.pleion.dev` rather than `models.pleion.dev` (a dedicated lessons host, not a route on the models service) — with `/l/<slug>` (a public page listing a lesson and its embed snippet), `/e/<slug>` (the bare page an iframe actually points at, with no `X-Frame-Options` so a third party can frame it), a fixed in-code lesson registry, and nine tests, all green. This package had no CI step of its own until it was found running headless in this pass — `pub get`/`analyze`/`test` are now wired into `tool/ci.sh` next to `cloud/server`'s own. Not done: an actual deployment behind that domain, and the literal acceptance below, which is a usage step once the page is live, not a code gap | cloud | M | edu-01 | the interactive is embedded in a Stepik course page and in a GitHub README through an iframe |
