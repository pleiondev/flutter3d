...
### edu-01: авторинг в редакторе — панель шагов закрыта, разборка перетаскиванием ещё нет

Панель написана 2026-09-12; строка таблицы поймана и сужена 2026-09-13 —
этот раздел ниже застал только первую половину и переписан вслед за
таблицей. Настоящая находка, определившая весь объём задачи панели:
...
**Разборка перетаскиванием сама по себе — не сделана**, и найдено 2026-09-13,
что она сведена не к арифметике одной, а к арифметике плюс отсутствующему
проводу с двух концов. `mergedOffsets(step, nodePath, delta)`
...
посчитать значение отдельно от виджета, который его вызовет — но саму
функцию сегодня не вызывает никто. С одного конца дело ближе, чем казалось:
`renderer.pickPixel` уже возвращает настоящий именованный `MeshNode` —
`model_instance.dart`'s `instantiate()` называет каждый узел по имени из
исходного glTF/`.f3d`, так что клик по `valve_cover` внутри одной модели
физически различим уже сегодня. Но `main.dart`'s `_handleUnder` эту связь
сразу же теряет: `dressing.handleFor(node)` идёт вверх по `node.parent`,
пока не найдёт запись в `SceneDressing.owners`, а туда попадает только
`instance.root` целиком — узел найден и тут же забыт ради целой сущности. С
другого конца дела нет вовсе: у `AxisDrag` нет собственного жеста под узел
(бары гизмо построены по габариту всей сущности, `gizmoBarsAround`), и он
пишет через `MoveBy` в `EntityDef.position`, а не через `SetField('offsets',
...)` в сущность шага — это два разных получателя команды, а не один и тот
же с другим полем. `movedStep` —
...
Не сделано: сама разборка перетаскиванием — `_handleUnder` не передаёт
имя узла наружу, у `offsets` нет собственного гизмо/жеста, и путь записи
для него не `MoveBy`, а `SetField('offsets', ...)` на другой сущности;
`editor_inspector.dart`'s дженерик-инспектор рисует Map (значит и
`offsets`) как `_ReadOnly` по собственному принципу «без падежа на поле» —
так что и вписать смещение вручную сегодня негде тоже. Дальше — как и
раньше: «получает ссылку» — механизма публикации урока (URL, хостинг) в
...
Не сделано, и по честным причинам, хотя не по той, что здесь стояла
раньше: `rp-01` с тех пор закрыт и доказал `Demo`/`.f3drun` живьём на всех
пяти — dungeon, platformer, racing и strategy's own `MatchDemo`, — так
формат больше не то, чего не хватает трём остальным жанрам. Настоящая
причина — соседняя: только у dungeon есть `RewindBuffer` в собственном
`main.dart` (`_rewind`, строка ~700) и колбэк `_remoteBugReport()`,
зарегистрированный через `registerTimelineExtensions(bugReport: ...)`
(строка ~398) — ни у platformer, ни у racing, ни у strategy нет ни
`RewindBuffer`, ни этой регистрации вовсе (`grep` по всем трём
приложениям не находит ни одного вхождения). Провести тот же провод для каждого —
не файловая ассоциация (она уже общая, `.f3drun` один тип на всех), а
завести свой `RewindBuffer` в каждом жанровом цикле шага — тот же
инстанс, что `rp-02`'s `RunTimeline` уже умеет обёртывать транспортом
для dungeon, просто ни у одного другого жанра нет самого объекта, который
можно было бы обернуть — отдельная работа по жанру, не подразумеваемая
приёмкой этой строки. **Drag-and-drop в
веб-версии — не
