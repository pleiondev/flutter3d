...
**Racing и strategy — по-прежнему без провода**, той же находкой, что и
раньше: ни `_beginDemo`/`_endDemo`, ни импорта `DemoFile` нигде в их
`lib/`. Racing к тому же не пользуется `RunSession`'s `onLost`-style
единой моделью так же прямо — у него нет `RunOutcome.lost` вовсе (см.
`run_outcome.dart`: «racing | countdown, running | — | finished»), только
`finished`, так что там `_endDemo` подключается к одному переходу, а не
к двум. Это следующий шаг того же пункта, не отдельная новая работа.
...
racing/strategy нет теперь только их половины (платформер сделан, см.
выше), и это не задача файловой ассоциации отдельно.
