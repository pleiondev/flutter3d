...
| rp-05 ⚠ | **Прогон в видео и в тест.** `dart run flutter3d:replay file.f3drun --video out.mp4` рендерит каждый кадр `renderFrame` через `cpu` и сшивает ffmpeg; `--check` сравнивает дайджесты и называет первый расхождённый шаг; `replayGolden()` в `testing` — golden-тест кадра N прогона (уже построен и используется). **⇢ «server that verifies a run by replaying it»** из ROADMAP, как самохостинг в CI проекта. **`dart run` буквально не собирается**: проверено эмпирически (пробным `bin/`-скриптом в `flutter3d_game_shooter`, а не только прочитано) — жанровый пакет реального прогона тянет `flutter3d_game`, а тот сам называет `flutter: sdk: flutter` в своём pubspec — не ради одного случайного типа, а по-настоящему: `src/input/desktop_input.dart`, `touch_controls.dart`, `touch_button.dart`, `touch_stick.dart`, `touch_slots.dart`, `touch_toggle.dart`, `playing.dart`, `config/accommodations.dart` — все реально импортируют `package:flutter/...` для жестов, клавиш и виджетов управления. Так что plain `dart run` падает прямо на компиляции фреймворка Flutter ещё до пользовательского кода — та же стена, что `flutter3d_sim_mcp`'s собственный pubspec уже документирует своей строкой «checked empirically, the same way `rp-05` found it out for genre packages generally» (эта находка была сделана раньше, но не долетела до этой строки плана). Единственный путь без правки жанровых pubspec — вести прогон изнутри `flutter test`, тем же приёмом, что `sim_mcp` использует для сокета вместо stdio: реальный CLI-инструмент, который просто `dart run`, для жанровой игры недостижим без пересмотра, откуда `flutter3d_game` берёт `WidgetBuilder` | build, testing | S | rp-01 | видео заезда 30 с из CI без GPU; тест «кадр 600 прогона такой же, как эталон» в одном из демо; `--check` на подменённой ленте называет шаг |
