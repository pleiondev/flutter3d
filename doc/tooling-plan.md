...
racing весь this-провод живёт в `_RaceScreenState`, приватном классе внутри
`main.dart`, и это не эта поправка что-то в этом устройстве меняет — весь
файл всегда был устроен так, для всех его пятнадцати с лишним подсистем
разом, а не только для этой. В репозитории нет ни одного теста, монтирующего
...
**Strategy — по-прежнему без провода**, той же находкой, что и раньше: ни
`_beginDemo`/`_endDemo`, ни импорта `DemoFile` нигде в её `lib/`. У неё вдобавок
свой собственный параллельный формат (`MatchDemo`/`OrderTape` из rp-01), не
`Demo`/`DemoFile` — так что этот провод для неё будет третьей отдельной
реализацией, не переносом ни dungeon/платформера, ни только что написанной
racing. Это следующий шаг того же пункта, не отдельная новая работа.

Не сделано, и по честным причинам: провод «игрок нажал кнопку в самой игре →
файл на диске → двойной клик открывает в редакторе» целиком — для strategy
нет теперь только её одной (платформер и racing сделаны, см. выше), и это не
задача файловой ассоциации отдельно.
