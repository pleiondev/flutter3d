...
| wg-02 | **Демо.** Терминал в крипте с журналом событий прогона; панель оператора у станка в шаблоне двойника (edu-05) | apps | S | wg-01 | обе сцены в галерее tpl-02 |
...
| tpl-02 | **Галерея живых демо.** Страница сайта с веб-сборками четырёх игр и сцен из гайдов; у каждой — «открыть в веб-редакторе» с уровнем по URL и «скачать прогон». Сборки — из CI, не руками | site, editor | M | net-03 | галерея открывается на телефоне; кнопка открывает уровень в веб-редакторе; гайд по свету встраивает живую сцену |
...
| edu-05b | **(закрыт 2026-09-15) `bindings`/`edu_data_source` — общий рендер, не только твин-приложение.** `edu-05`'s собственная запись закрыла механизм данных (`DataSourceRegistry`/`resolveBindings`) и `apps/flutter3d_lab_twin`'s частный провод в один `ValueNotifier`; `lesson_player.dart`'s честная запись о нереализованности при этом оставалась верной — общего способа записать `resolveBindings`'s результат в узел сцены не было нигде. `applyLessonStepBindings` (`packages/flutter3d_bridge/lib/src/lesson_player.dart`) — обратная сторона `resolveBindings`, ровно то, что `doc/edu-00-interactive-format.md` §9 называет «путь сущность.путь-свойства... что значение, не куда записано» — теперь есть и «куда»: `"entity.at"`/`"entity.yaw"` (трансформ) и `"entity.material.<field>"` (`emissiveColor`, `emissiveStrength`, `roughness`, `metallic`, `baseColor`) на именованном узле. Неизвестный узел или поле материала — пропускается, не бросает, тот же принцип, что `offsets`'s разбор уже держит. 7 новых тестов в `lesson_player_test.dart`: запись трансформа, запись материала, смена значения между шагами, узел не найден, узел не `MeshNode`.

**Обновление того же дня: подключено и в `flutter3d_lesson_viewer`.** Формат
сам не описывает синтетическую форму сигнала для `kind: "sampler"` — это
решение хоста, как `apps/flutter3d_template_app`'s `60 + 15·sin(...)` уже
решает для своего твина. Для общего проигрывателя, который не знает, что
означает источник с любым именем, решение то же по духу и по буквальному
контракту `kind: "sampler"` (`doc/edu-00-interactive-format.md` §9:
«синтетический источник для демо и тестов без реального брокера») — один
общий сигнал `{"value": 50 + 10·sin(step·0.05)}` для ЛЮБОГО источника с
этим `kind`, а не отдельная семантика на источник. `LessonCubit.open`
читает сущности `edu_data_source` документа, строит `DataSourceRegistry` с
таким сэмплером на каждый `kind: "sampler"` и репортит через `onIssue`
(новый необязательный параметр) любой другой `kind` — `mqtt`/`websocket` не
имеют адаптера в этом обобщённом проигрывателе, и это честно сказано, а не
молча дало пустое значение. `_LessonScreenState`'s собственный тикер (тот
же, что уже дёргает `widgetSurfaces.tickAll()`) каждый кадр вызывает
`applyLessonStepBindings` для текущего шага плеера со счётчиком,
независимым от индекса шага — та же граница, что `_twinStep` уже проводит
в `template_app`. `assets/levels/teardown-demo.json` получил
`edu_data_source`/`bindings`, пишущие `engine-body.material.emissiveStrength`
на первом шаге; новый `assets/levels/unsupported-source-demo.json` —
фикстура для случая `kind: "mqtt"`. 2 новых теста в `lesson_cubit_test.dart`.
| bridge, apps | S | edu-05 | `applyLessonStepBindings` пишет `material.emissiveStrength` реального `MeshNode` от значения, которое `resolveBindings` реально прочитал из `SamplerDataSource` на двух разных шагах, с разными числами, и в `flutter3d_lesson_viewer` то же самое подключено через `LessonCubit`, не только вручную в тесте |
...
| edu-07a | **(закрыт 2026-09-15) Путь (б): `prop` — лёгкая адресуемая геометрия.** Найдено готовя `ls-e-00`/`ls-e-04`/`ls-i-01`/`ls-i-03` (см. журнал сессии): ни браш (батчится по материалу — `configurator.json`'s собственный докстринг уже называет это «retints the panel's own display, not the product's mesh»), ни `type: "model"` (не читается `LevelLoader` вообще) не давали документу ни одного именованного, индивидуально адресуемого куска геометрии. `PropVisuals` (`packages/flutter3d_bridge/lib/src/prop_visuals.dart`) — новая сущность `type: "prop"` (box/sphere/cylinder, материал по имени из `level.materials`), своя `MeshNode` на сущность, не батчится; `setMaterial` меняет материал живьём. Подключено в `flutter3d_lesson_viewer` (`main.dart`/`LessonView`/`OrbitCubit`) взаправду, не только в тестах: новый урок `assets/levels/teardown-demo.json` реально двигает `prop` между шагами через `offsets` в настоящем `LessonView` (`test/prop_offsets_test.dart`, `tester.tap` по кнопке «Next», позиция узла проверена до/после). Не замена реальной модели — плейсхолдерная геометрия для контента, который её ждёт | bridge, apps | M | edu-00 | `ls-i-01`'s аннотация реально красит продукт (`setMaterial`, протестировано); `ls-e-00`-стиль урон с плейсхолдерной геометрией реально двигается по шагам в проигрывателе |
| edu-07b | **(закрыт на уровне механизма 2026-09-15 — см. ниже) Путь (а): `LevelLoader` грузит `type: "model"` (внешний glTF/`.f3d`).** Единственный путь к НАСТОЯЩЕЙ модели (не плейсхолдеру `prop`) для `ls-e-00`'s собственного двигателя: модельная загрузка в этом дереве идёт через изолят-безопасный `ModelAsset.instantiate`, а не тривиальную функцию, которую можно вызвать в лоб из `LevelLoader.build` — нужен асинхронный путь с оглядкой на изолят. `edu-07a`'s `prop` закрывает механику (offsets/материал двигаются), но не подменяет реальный ассет | sim, bridge, apps | L | edu-07a | `ls-e-00`'s пять шагов двигают деталь настоящей модели (glTF/`.f3d`), не коробку-плейсхолдер |
| edu-07c | **(закрыт 2026-09-15) Мост `edu_annotation` в рендер.** Пробел, найденный в журнале сессии tpl-04 («ни один рендер-путь не читает `attachTo` и не превращает его в позицию `WidgetSurface`»), закрыт: `WidgetSurfaceVisuals.add` (`packages/flutter3d_bridge/lib/src/widget_surface_visuals.dart`) дважды диспетчерит по `entity.type` — `widget_surface` (старый путь, `at`/`yaw`) и `edu_annotation` (новый: `attachTo` разрешается через `nodes: Map<String, SceneNode>`, `offset` ставится как локальная позиция, построенная поверхность добавляется РЕБЁНКОМ найденного узла, а не сестрой в мировых координатах — так что аннотация едет вместе с узлом, который сдвигают `offsets`). Неизвестный `attachTo` репортится через `onIssue`, как и неизвестный `widget`, а не бросает исключение. `apps/flutter3d_template_app` и `apps/flutter3d_lesson_viewer` строят `PropVisuals` первым проходом и передают его `nodes` в `widgetSurfaces.add`, чтобы `attachTo` мог назвать `prop`, прочитанный раньше или позже по документу. `configurator.json`'s "config-panel" — теперь `edu_annotation`, `attachTo: "product"`, не свободный `widget_surface` | bridge, apps | S | edu-07a, wg-01 | `ls-i-01`'s аннотация в буквальном смысле формулировки: `edu_annotation`, а не отдельная панель |
| edu-07d | **(закрыт на уровне механизма 2026-09-15) `ModelVisuals`: `type: "model"` через реальный isolate-safe glTF/`.f3d`.** `packages/flutter3d_bridge/lib/src/model_visuals.dart` читает `entity.string('asset')` (путь §6/§11 формата уже называет), декодирует через `decodeModelInIsolate` и грузит через `ModelAsset.fromDocument` — тот же путь `fixture_visuals.dart`'s `_load` уже проверил на настоящих уровнях, но здесь `await`ится, а не «выстрелил и забыл»: шаг урока читает положение узла в момент открытия документа, значит модель обязана быть в сцене к этому моменту, а не двумя кадрами позже. Именованный узел ВНУТРИ модели (`ModelNode.name`) адресуем как `"entity#node"`, тот же формат §6 уже называет для `offsets`/`attachTo` — не только корень целиком. `apps/flutter3d_lesson_viewer/lib/main.dart` строит `props` и `models` вместе, `await Future.wait` каждой модели до `LessonReady`, и передаёт объединённую карту узлов в `WidgetSurfaceVisuals`/`LessonView`. Семь тестов на уровне `flutter3d_bridge` (`test/model_visuals_test.dart`, `FileAssetSource` на реальный `.glb` из `flutter3d_samples` — не подделка бандла Flutter: happy path, адресация `entity#node`, движение по `offsets`, чужой тип, отсутствующий `asset`, файл, который не decode'ится, dispose) и один сквозной через настоящий `LessonView` (`apps/flutter3d_lesson_viewer/test/model_offsets_test.dart`, `tester.tap` по «Next»/«Previous» реально двигает корень декодированной модели). Не сделано: `prep-00` — выбор реального узла для разборки остаётся отдельной, ничем не закрытой предметной задачей; `ls-e-00`'s буквальная приёмка ждёт именно контент, не код | bridge, apps | M | edu-07b | `model_offsets_test.dart` зелёный: настоящий `.glb`, декодированный и загруженный через `decodeModelInIsolate`, реально двигается между шагами настоящего `LessonView` |
...
†dungeon исправлен 2026-09-15 — см. ниже; после исправления k=8 стоит 0.52 мс,
не 4.74. Строка выше оставлена как первый замер, а не переписана, потому что
разница между «дорого нашли» и «дорого исправили» — часть самой записи.

**Бюджет держали racing и platformer с большим запасом с самого начала;
dungeon был на грани и теперь тоже держит его с запасом; strategy —
единственный, кто не укладывается (почти в четыре раза), и по причине,
которую гистерезис ниже не трогает — крауд/экономика/боты, а не сетка
навигации.**
...
- **dungeon — найдено и исправлено 2026-09-15.** Оба кандидата, названные
  выше («Automap.reveal», пересборка `CollisionWorld`), оказались НЕ ПРИ ЧЁМ
  — проверено не рассуждением, а прямой инструментацией (`Stopwatch` вокруг
  каждого вызова внутри `CollisionWorld.update()` и `Automap.reveal()`,
  временная, снята после находки): ни один не показал ни одного дорогого
  вызова во время измерения. Настоящая причина — тремя уровнями глубже, в
  `ActorSystem.step` → `Navigation.update` → `FlowField.update`: цель поля
  потока (позиция игрока) дребезжала между двумя соседними ячейками
  навигационной сетки (`packages/flutter3d_game_shooter/lib/staging.dart`'s
  `cellSize: 0.25`) на величину около 2,7 см — обычное дрожание физики у
  стены, а не баг отката — и `FlowField.update`'s собственная оптимизация
  «пересчитывать только когда цель сменила ячейку» превращала это дрожание в
  ПОЛНЫЙ пересчёт Дейкстры по всей сетке на КАЖДОМ шаге, пока игрок стоял
  в этом дрожании. Не специфично для отката: то же дрожание в обычной игре
  без единого `restore()` платило бы ту же цену на каждом шаге, где враг
  преследует игрока рядом со стеной — откат просто оказался тем, что это
  заметило первым.

  **Исправлено добавлением гистерезиса**
  (`packages/flutter3d_sim/lib/src/nav/flow_field.dart`): `FlowField` хранит
  позицию цели на момент последнего настоящего пересчёта
  (`_lastSweepGoal`), и смена ячейки доверяется только когда цель прошла
  не меньше трети размера ячейки от той позиции — метраж дрожания привязан
  к физике (доли сантиметра — метры), а не к размеру сетки, поэтому десятая
  доля ячейки (изначальная попытка) не пережила проверку на `cellSize:
  0.25`, а треть — с запасом. Новый регрессионный тест
  (`packages/flutter3d_game_shooter/test/navigation_test.dart`, «a goal
  jittering across a cell edge does not re-sweep every step») находит
  настоящую границу ячейки через `grid.cellAt` (не угадывает по формуле
  начала координат), дрожит на миллиметр по обе стороны двадцать раз подряд
  и проверяет, что `field.goalCell` не меняется, затем что настоящий переход
  на треть ячейки дальше всё ещё пересчитывает. `dart test` в
  `flutter3d_sim` — 549 тестов, зелёные; `flutter test` в
  `flutter3d_game_shooter` — 349, зелёные (включая тесты детерминизма и
  снимков — гистерезис не тронул то, что переигрывается побитово);
  `flutter3d_demo_dungeon` — 124, зелёные; `flutter3d_game_platformer` (тоже
  держит `Navigation`) — 221, зелёные.

  **Числа после исправления**, тот же тест, та же машина: k=1 0.49 мс, k=2
  0.55 мс, k=3 0.39 мс, k=4 0.50 мс, k=5 0.51 мс, k=6 0.53 мс, k=7 0.55 мс,
  k=8 0.52 мс (бюджет 4 мс), k=9 0.51 мс, k=10 0.52 мс — плоско и на
  порядок ниже бюджета вместо 4,5–15 мс до исправления, и «на грани» из
  таблицы выше больше не описывает эту строку.

Не измерено, тем же ограничением, что и у rp-00: настоящий телефон среднего
класса. Числа dungeon выше — уже с исправлением; strategy и его отдельная,
не связанная с этой находкой линейная цена (крауд/экономика/боты каждый
тик) остаются как были — нижняя граница того, что бюджет потребует; на
устройстве медленнее они будут только хуже.
...
### par-01: hot reload моделей и текстур — вне охвата этой волны, честно

Не начат, и не по забывчивости: единственный пункт во всём этом документе
(rp-/net-/ai-/wg-/par-/tpl-), у которого нет раздела статуса, потому что у
него нет попытки. Приёмка сформулирована через модельер буквально —
«модель, сохранённая в модельере, появляется в демо на телефоне без
перезапуска» — а модельер (`apps/flutter3d_modeler` и всё, что пишет в его
`hook`/сохранение) отдельным поручением выведен из этой волны: она про
финализацию треков **кроме** модельера и учебника. `par-01` строится на
`ap-05`/`ap-11` (сами по себе закрыты, из `doc/asset-pipeline-plan.md`), но
проверить приёмку без строки кода в модельере невозможно — это не
пограничный случай, а прямое попадание в исключённый трек. Оставлен
нетронутым, чтобы не столкнуться с параллельной сессией, которая этот
трек и ведёт.
...
(`flutter3d_cpu/test/hdr_readback_test.dart`), который пином держит оба
...
**Обновление 2026-09-15: `offsets` (§6) достигло паритета с плоским
проигрывателем.** `edu-07a`/`edu-07d` дали `flutter3d_bridge`'s
`applyLessonStepToCamera` слоистую разборку уже ПОСЛЕ того, как этот
пункт был закрыт — `flutter3d_stereo`'s собственная `applyLessonStep`
осталась при исходных `visible`/`hidden`, без офсетов, тихо отстав от
файла, который сам называет своим шаблоном
(`apps/flutter3d_lesson_viewer/lib/src/lesson_view.dart`'s докстринг:
«`flutter3d_stereo/test/lesson_stereo_view_test.dart` already uses to
avoid a GPU»). Теперь `applyLessonStep`/`LessonPlayer.applyCurrent`/
`LessonStereoView` несут тот же `restPositions`-параметр и ту же логику
разбора `offsets`, что и `flutter3d_bridge` — по-прежнему собственная
копия, не общая зависимость (докстринг файла сам объясняет почему: риг и
плоская камера — разные примитивы рендера, и у каждого файла нет причины
знать о рендерере другого). 2 новых теста в `lesson_player_test.dart`
(узел с офсетом реально сдвигается; узел без записи в шаге возвращается
к исходной позиции — не диф). Значит `ls-x-00`/`ls-x-01`/`ls-x-03`
(`edu-06`'s единственная зависимость) не унаследуют разрыв в возможностях
между «то же самое в стерео» и тем, что плоский `ls-e-00`/`ls-i-00` уже
умеют.

### edu-01: обновление 2026-09-15 к записи «закрыт» выше (§4.7)

**Предупреждение себе, не находка о коде.** Этот день начался с того, что
`edu-01` был назван (в этом же разговоре, несколько раз подряд) «крупная
фича редактора, не начато» — при том что запись прямо выше, датированная
2026-09-12, уже говорит «закрыт, без единой новой команды», подробно
называет `step_panel.dart`, `mergedOffsets`, гизмо через обычный поворот
сущности. Источник ошибки — суждение о коде по устному пересказу
предыдущего ответа, а не по самому файлу; то же самое, что нашлось у
`check_prompt.dart` в начале этой сессии. Урок для следующего раунда:
`grep` по дереву раньше, чем по памяти разговора.

**Что реальная проверка добавила к записи от 2026-09-12.** `mergedOffsets`
(чистая арифметика, 14 тестов) существует и проверен — но НИГДЕ в
`apps/flutter3d_editor/lib/` его никто не вызывает: сама функция готова,
драг-жест, который считал бы `delta` и передавал её сюда, не написан.
Это и есть честный остаток «разборки перетаскиванием» — не «нужна новая
архитектура», как было сказано раньше в этом же разговоре, а «есть
арифметика, нет одного вызова из UI». «Гизмо среза» — не остаток вовсе:
запись от 2026-09-12 уже объясняет, что вращение через `,`/`.` для любой
выбранной сущности — это и есть решение, отдельный 3D-хэндл сознательно
не строился.

Отдельно, реальный маленький пробел внутри уже закрытого: `_addClipPlane`
никогда не задавал `normal`, а `Editing.offerable` для сущностей
намеренно пуст — значит поле никогда не появлялось в инспекторе для
редактирования вообще, даже с гизмо-через-поворот из записи выше.
Исправлено: `_addClipPlane` теперь задаёт `normal: [0, 0, 1]` при
создании. 1 новый тест в `step_panel_test.dart`.

**Побочная находка, не про формат.** Полный прогон `flutter3d_editor`'s
тестов (не запускавшийся ни разу за всю сессию, пока это не понадобилось)
нашёл настоящий регресс от более раннего момента этой же сессии:
`apps/flutter3d_template_app/lib/main.dart` менялся несколько раз
(`edu-07a`/`edu-07c`), но `apps/flutter3d_editor/assets/templates/*/
app.main.dart.txt` — байт-в-байт копии этого файла, которые
`scaffold_test.dart` сверяет на равенство ("A `main.dart` that is only
ever a string in a scaffolder is one that stops compiling six months
later") — не обновлялись ни разу. Все четыре шаблона
(shooter/platformer/racing/strategy) синхронизированы заново. Урок:
изменение файла, который читает другой пакет через `File(...).
readAsStringSync()` в собственном тесте, требует прогона ЭТОГО пакета,
не только пакета, где лежит изменённый файл — правило, которое
`tool/structure.dart`'s собственный `no test builds its own world` не
покрывает, потому что оба теста честно строят мир из настоящих файлов.

**И ещё одна, про сам счётчик тестов.** Статический подсчёт (`grep`
`test(`/`testWidgets(` по исходникам — то, что `tool/structure.dart`'s
правило действительно значит под «documents says how many tests there
are») даёт для `flutter3d_editor` 206, не 203/204: план уже был устаревшим
на 2 теста ДО этой сессии, независимо от неё. Исправлено в счётчиках
ниже §13 наравне с реальным приростом этого дня.

### edu-01: 2026-09-15, продолжение — «нет одного вызова из UI» закрыто

Ровно тот остаток, который запись выше честно называла: `mergedOffsets`
существовал и был проверен, но драг-жест, который считал бы `delta` и
передавал её туда, написан не был. Теперь есть.

**`Editing.activeStepForOffsets`** (`editing.dart`) — какой `edu_step`,
если такой выбран, нажатие стрелки пишет в `offsets` вместо того, чтобы
двигать выбранную сущность. Режим сессии, не поле документа — переключается
новой кнопкой (`Icons.open_with`) в каждой строке `StepPanel`, гаснет сама
при повторном нажатии той же строки или при нажатии другой (одна активная
строка, не накопление). **`Editing.nudgeOffset(Vector3)`** делает
собственно запись: читает текущий офсет выбранного узла из активного шага
(`_offsetOf`), складывает с `delta`, проводит через уже существующий
`mergedOffsets`, и пишет назад через `setField('offsets', ...)` — с
временной сменой выделения на сам шаг и восстановлением её после, потому
что `setField` пишет в то, что выбрано сейчас, а нужный адресат — шаг, а
не выбранная деталь.

**`NudgeOffset extends EditorCommand`** (`editor_command.dart`, одиннадцатая
запись в `editorCommandNames`) — то единственное отступление от заголовка
записи 2026-09-12 («без единой новой команды»): `main.dart`'s собственный
комментарий у обработчика клавиш прямо говорит «каждая клавиша, меняющая
документ, идёт через историю», и `editing.nudgeOffset(...)` в обход
`history.run(...)` был бы вторым, необратимым способом менять документ —
именно то, для чего сама история была написана. Одиннадцатая команда — цена
того, чтобы это осталось правдой, а не архитектурный откат: команда ничего
не делает сама, только оборачивает тот же `nudgeOffset`.

`apps/flutter3d_editor/lib/main.dart`'s обработчик стрелок теперь пробует
`NudgeOffset` первым (если `activeStepForOffsets` не пуст и команда
применилась), иначе — как раньше, `MoveBy`.

**Побочная находка, серьёзнее самой фичи.** Тест на `NudgeOffset` с undo
(«отменить нажатие — офсет должен пропасть») падал не из-за нового кода:
`Editing.setField` документировал себя как «правит строку на месте»
(«Live rather than a copy... edits it in place»), а `Level.toJson()`
(через `writeThrough`'s собственную оптимизацию — не переписывать то, что
не изменилось) отдаёт для неизменённой записи тот же самый объект `Map`,
что и предыдущий снимок. `EditorHistory.transaction` берёт снимок «до»
через тот же `level.toJson()` перед вызовом команды — значит для ПЕРВОГО
изменения любого поля через `SetField` (не только `NudgeOffset`) снимок
«до» и документ, который `setField` правит «на месте», были одним и тем же
объектом в памяти. `undo()` восстанавливал документ, в котором изменение
уже стояло. Минимальный воспроизводящий пример без `NudgeOffset` вообще —
`history.run(SetField('solid', false))`, затем `undo()` — подтвердил, что
баг не новый и не про `edu-01`: он был в `setField` с самого начала,
просто ни один существующий тест не проверял ровно эту комбинацию (undo
после первого `SetField` на нетронутом документе). Исправлено: `setField`
теперь клонирует список и саму строку перед правкой, а не мутирует то,
что вернул `toJson()`, — контракт «на месте» из старого комментария
`_rowOf` был неверен и убран.

Новые тесты: 4 в `editor_command_test.dart` (`group('NudgeOffset')` —
без активного шага, когда выбор — сам шаг, запись + восстановление
выделения + undo, слияние с уже существующим офсетом; последний из них
и поймал баг выше), 3 в `step_panel_test.dart` (кнопка включает/выключает
режим для своей строки, переключение на другую строку). Затронутые пакеты
пересчитаны в §13 (`flutter3d_editor_core` 126→130, `apps/flutter3d_editor`
206→209).

Что не тронуто и не должно быть: гизмо среза (уже решено поворотом,
запись 2026-09-12), кросс-бэкенд рендеринг `edu_clip_plane` (см. отдельную
запись — блокируется отсутствием GPU в этом окружении, не объёмом кода),
живая регистрация в браузере (см. `edu-03-lti-plan.md` §8).
...
**Не сделано, названо прямо (обновлено 2026-09-15 — три абзаца выше устарели
на день своего же написания):**
- ~~`offsets` не подключено ни к одному настоящему приложению~~ — устарело:
  `edu-07a`/`edu-07d` (выше в этом же файле) подключили и `prop`, и
  `type: "model"` (через новый `ModelVisuals`, изолят-безопасная загрузка,
  та самая, которую этот абзац называл «отдельным пунктом») в
  `flutter3d_lesson_viewer`/`flutter3d_template_app`, оба со сквозными
  тестами через настоящий `LessonView`/`tester.tap`. `edu_clip_plane`,
  `bindings`/`edu_data_source` в этом дереве по-прежнему честно не входят,
  по докстрингу самого `lesson_player.dart`.
- ~~`edu-03` (LTI/xAPI) не начат~~ — устарело: `packages/flutter3d_lti`
  (OIDC-запуск, JWKS-проверка, AGS, xAPI) и сервис `cloud/lti` построены и
  проверены против настоящей Moodle-песочницы (`doc/edu-03-lti-plan.md`).
...
**Поправка от 2026-09-15: платформер получил свой собственный провод.**
`_beginDemo`/`_endDemo` (буквально те же имена и тот же механизм, что уже был
у dungeon) добавлены в `apps/flutter3d_demo_platformer/lib/main.dart`, но не
скопированы бездумно — архитектура другая, и это нашлось при переносе, а не
предположилось заранее:

- У платформера нет `BlocConsumer` с `listener:` для смены уровня —
  `onLevelBuilt`, колбэк из `RunSession.open`, и его сигнатуре не хватало
  `asset` (только `LevelReady`/`GraphicsDevice`), потому что раньше он был
  нужен только для камеры и раннера. `rp-01`'s `_beginDemo(asset, level)`
  этого не может без имени файла — добавлен третий параметр в
  `PlatformerRun.onLevelBuilt`'s тип, единственная точка вызова в
  `run.dart` правлена той же строкой. Оба существующих теста, строившие
  `PlatformerRun` напрямую (`run_test.dart`, `run_cubit_test.dart`), —
  поправлены под новую сигнатуру; в `run_test.dart` добавлен новый тест
  («onLevelBuilt names the level it just built, not the one it's
  leaving»), которого не было даже для старой сигнатуры.
- Конец уровня, наоборот, у платформера ЕСТЬ через общий механизм —
  `RunSession.observe()` уже республикует `RunPlaying` с новым `outcome`
  ровно один раз на переход (не на кадр, пока переход держится), тем же
  `onChanged`, что `RunCubit` уже проводит в `emit`. Так что `_endDemo`
  подключён не копией dungeon-овского `_wasFinished`-паттерна из
  `soundtrack.dart`, а честно — через `BlocConsumer`'s `listener:`
  (`BlocBuilder`, которым раньше был весь `build()`, стал `BlocConsumer`
  с добавленным `listener:`, `builder:` не тронут), на переходы
  `RunOutcome.lost`/`RunOutcome.won` — тот самый общий `RunOutcome`,
  документированный в `run_outcome.dart`'s собственной таблице для всех
  трёх жанров разом, а не выведенный заново для платформера.
- Чекпоинт (`DigestTrace.observe`) — в `_step`, сразу после `sim.step(dt)`,
  той же строкой, что у dungeon.

Полный набор — `flutter analyze` чисто, `flutter test` — 231 тест (было
230, +1), включая настоящий `playing_the_game_test.dart` (реальный виджет,
`CpuDevice`, играется через тачскрин-контролы) без единой регрессии. Формат
уже был проверен per-genre в `demo_test.dart` (`rp-01`) — этот провод
проверяет только то, чего раньше не было: что `main.dart` действительно
вызывает `_beginDemo`/`_endDemo` в нужный момент, а не то, что `Demo` сам
по себе работает. Отдельного виджет-теста «сыграл до смерти → файл на
диске» нет — той же честной причине, по которой его нет и у dungeon:
`DemoFile` использует настоящее хранилище без внедряемой альтернативы
(тем же способом, каким `SettingsFile`/`SaveFile` уже устроены в этом
приложении), так что проверка записи на диск для существующего провода
тоже осталась там, где и была — не новый пробел, тот же самый.

**Та же поправка от 2026-09-15: racing получила свой провод тоже, и не тем же
кодом, потому что у неё не тот же скелет.** `_beginDemo`/`_endDemo` теперь и в
`apps/flutter3d_demo_racing/lib/main.dart`, но без `BlocConsumer`/`listener:`
вообще — у racing нет `RunSession`, а `RaceProgress` (см. её собственный
докстринг в `race_cubit.dart`) прямо называет себя тем, чем `RunSession` был
бы «с вырезанной серединой»: три перехода вместо пяти, никакого снимка,
никакого `restoreInto`. Как раз потому, что у неё нет очереди статусов, через
которую слушать переход, оба конца провода — это два прямых вызова там, где
переход и так уже происходит императивно:

- `_beginDemo` зовётся в `_loadCircuit`, сразу после `_raceCubit.ready()` —
  тем же местом, что `_beginDemo(asset, level)` у платформера, только имя
  трассы и её хеш не читаются из уже посчитанного `LevelReady`, потому что
  `TrackDocument` не хранит ни то, ни другое. `contentDigestHex` (тот же,
  которым `Level.digestHex` считается для двух других жанров, и тот же,
  которым его уже считал `test/demo_test.dart` для этой самой трассы) теперь
  зовётся и в `main.dart` — от JSON, который `_loadCircuit` и так уже
  декодирует для `TrackDocument.fromJson`, сохранённого в переменную вместо
  повторного разбора.
- Конец, наоборот, ни разу не переход статуса — это прямой вызов
  `_finishedHere()`, единственное место, где вообще решается, что круг
  выигран. `_endDemo()` зовётся первой строкой в нём, до
  `_raceCubit.finish(...)`, потому что оба читают разные вещи с трассы: один
  — то, что уже произошло, другой — что произойдёт дальше.
- `_startOver()` — единственное место, где можно уйти с трассы, не выиграв
  её, — никогда не вызывается посреди круга: клавиша R активна только когда
  `_seasonIsOver`, а с экрана `RaceFailed` уходить нечего — там ещё не было
  никакой трассы, чтобы начать её запись. Так что не осталось случая, когда
  `_beginDemo` был бы вызван, а `_endDemo` за ним — нет; `_beginDemo` всё
  равно защитно зовёт `_endRecording()` первой строкой, как и у двух других
  жанров, а не полагается на этот факт.
- Чекпоинт — в `_driveOneStep`, сразу после `simulation.step(stepSeconds)`,
  тем же кодом, что у платформера.

`flutter analyze` чисто, `flutter test` в `flutter3d_demo_racing` — 188
тестов, без единой регрессии (число тестов не изменилось: новых тестов
racing не получила — см. ниже, почему). `dart run tool/structure.dart` —
32/32, без правки чисел: только `run_test.dart` у платформера добавил тест,
здесь ни один файл с числом тестов не тронут.

**Почему без нового теста, в отличие от платформера.** У платформера
`_beginDemo`/`_endDemo` были добавлены поверх уже тестируемого класса
(`PlatformerRun`, драйвится напрямую в `run_test.dart` без монтирования
виджета) — там новый параметр `onLevelBuilt` было чем проверить дёшево. У
racing весь этот провод живёт в `_RaceScreenState`, приватном классе внутри
`main.dart`, и не эта поправка устроила его так — весь файл всегда был
устроен так, для всех его пятнадцати с лишним подсистем разом, а не только
для этой. В репозитории нет ни одного теста, монтирующего
`RaceScreen`/`RacingApp` (`net_race_screen_test.dart` монтирует другой экран,
`NetRaceScreen`, для мультиплеера) — то есть планка здесь та же, что и у
dungeon с платформером: механизм (`Demo`/`DemoFile`/`InputTapeRecorder`)
проверен `demo_test.dart` per-genre; сам провод — только сборкой и полным
прогоном тестов без регрессии, потому что для проверки самого вызова
`_beginDemo`/`_endDemo` изнутри `_RaceScreenState` понадобилась бы
инфраструктура монтирования, которой это приложение никогда не имело — не
новый пробел, тот же самый, только без готового `PlatformerRun`-класса,
на который можно было бы его списать в этот раз.

**Та же поправка от 2026-09-15, последняя четверть: strategy получила свой
провод тоже, и по дороге нашёлся пробел в rp-01 самом, не только в rp-04.**
У этого жанра свой собственный параллельный формат — `MatchDemo`/`OrderTape`
(не `Demo`/`InputTape`, см. `order_tape.dart`'s собственный докстринг о
том, почему), написанный ещё в rp-01. Но `apps/flutter3d_demo_strategy/test/`
не имело `demo_test.dart` вообще — ни одного. Три другие демки давно
проверяют свой формат прогоном по-настоящему отгруженной карты/уровня, а
у strategy весь тираж проверок (`packages/flutter3d_game_strategy/test/
tape_test.dart`) — на голой `StrategySimulation`, собранной в коде, и без
единого теста, что записанная лента реально переигрывает настоящий матч на
`assets/levels/map_a.json` до того же байта. Формат был проверен по частям
(JSON туда-обратно, все четыре отказа), но не целиком — и целиком означало
найти вопрос, на который в существующем коде ответа не было.

**Найденный вопрос: как переигрывать матч с ботом, не удваивая его
заказы.** `Match.step` безусловно зовёт `bot.step(simulation)` для каждого
бота, потом `simulation.step(dt)` — без способа сказать «не спрашивай бота
в этот раз». Лента же (`OrderTapeRecorder`, повешенный на
`StrategySimulation.orders.recorder`) ловит буквально всё, что легло в
очередь тем шагом, ботом ли, кликом ли — прямо по докстрингу самого
`recorder`а: «сеет всё, что дают оба, и ни одному не нужно знать, что
кто-то смотрит». Значит переигрывание тем же `Match.step` заставило бы
бота решать заново поверх состояния, которое его первое решение уже
произвело, и добавило бы в тот же шаг заказы дважды. Разгадка — в
собственной сигнатуре `OrderTapePlayback.applyTo`: она принимает
`OrderQueue`, не `Match`. Формат с самого начала был написан для
переигрывания на уровне симуляции, а не матча — просто это нигде не было
проверено и поэтому не было явным. Так что `demo_test.dart`
(`apps/flutter3d_demo_strategy/test/demo_test.dart`, новый файл) пишет и
переигрывает на уровне `StrategySimulation`, а не `Match`: на живом
прогоне бот и `restock()` зовутся вручную тем же порядком, что и внутри
`Match.step` (что даёт возможность повторить именно эту половину при
переигрывании и без другой), а при переигрывании — только
`playback.applyTo(simulation.orders)` и `simulation.step(dt)`, без единого
обращения к боту. `MatchDemo.start` — соответственно снимок
`StrategySimulation`, не `Match` (голова бота, `Bot.save`, туда не идёт —
по той же причине, по которой она нужна только сохранению между запусками,
а не прогону, который сохранением не является). 600 шагов настоящей карты,
контрольные точки через `DigestTrace` — тот же протокол, что у трёх других
жанров.

**Провод в самом виджете — поверх уже проверенного формата.**
`onLevelBuilt` в `apps/flutter3d_demo_strategy/lib/src/run.dart` получил
два новых параметра (`String asset, String levelHash`) вместо одного
`Staged` — оба нужны `_beginDemo`, и ни один не переживает `open` иначе:
`Staged` хранит собранный матч, а не `StrategyMap`, из которого он собран.
`levelHash` — `map.level.digestHex`, то же самое поле `Level`, которым
считают его и три других жанра. Вызов `onLevelBuilt(asset, ...)` — там же,
внутри `StrategyRun.open`, откуда и раньше. **Тот же пробел, что у
платформера, и по той же причине: `_beginDemo` идёт из `onLevelBuilt`,
который срабатывает до `restoreInto` — `RunSession.load` зовёт `restoreInto`
только после того, как `open` уже вернул уровень.** Названо прямо в
докстринге `_beginDemo` в `main.dart`, той же фразой, что у платформера: для
матча, возобновлённого из сохранения, демо начинается с открывающей
карту толпы, а не с той, что реально была на сохранении. Конец, наоборот, —
не через слушатель статуса (у strategy нет ни `Cubit`, ни `BlocBuilder`
вовсе — `main.dart`'s собственный докстринг называет весь экран написанным
без BLoC), а прямым вызовом `_endDemo` из `_keep`, там же, где `run.isOver`
уже читается ради `run.advance()` — второе чтение того же поля ничего не
стоит, а `_endDemo` сам по себе пишет только один раз (рекордер — `null`
на каждый вызов после первого).

`MatchDemoFile` — новый маленький класс
(`apps/flutter3d_demo_strategy/lib/src/match_demo_file.dart`), не
переиспользование `DemoFile`: тот типизирован на `Demo`, чьё поле `tape`
по имени зовёт `InputTape.fromJson` — ровно то, от чего `order_tape.dart`
и отказался в rp-01, тем же аргументом. Один читатель, один документ.

`flutter analyze` чисто, `flutter test` в `flutter3d_demo_strategy` — 44
теста (было 43, +1 — новый `demo_test.dart`), без единой регрессии;
`flutter3d_game_strategy`'s собственный пакет — тоже без изменений и тоже
зелёный. `dart run tool/structure.dart` — 32/32 после правки числа тестов
в четырёх документах и строки-суммы таблицы в `testing.md`.

**Этим закрывается rp-04 по всем четырём жанрам разом** — dungeon (было
готово раньше), платформер, racing и strategy, каждый своим кодом под свою
архитектуру, а не одной копией на всех: `BlocConsumer`-слушатель на
`RunOutcome` у платформера, два прямых вызова у racing (нет `RunSession`
вообще), прямой вызов из `_keep` у strategy (нет `Cubit` вообще). Один и
тот же пробел на «начале» у платформера и strategy (`onLevelBuilt` до
`restoreInto`) назван и не скрыт ни там, ни там.

Не сделано, и по честным причинам: провод «игрок нажал кнопку в самой игре →
файл на диске → двойной клик открывает в редакторе» целиком связан с двумя
вещами отдельно от жанров — не задачей файловой ассоциации.
**Drag-and-drop в веб-версии — не
