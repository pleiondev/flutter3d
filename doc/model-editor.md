...
| pro-sc-01 мазок на 1,2 млн | macOS: открытие+BVH 3,3–5,7 с (порог 3 с не пройден); мазок (отбор+правка+нормали+overwrite+raycast) 294–1005 мс (пороги 8/16 мс не пройдены в 20–60 раз, узкое место — полный пересчёт нормалей). Chrome: открытие+BVH 24,2–24,4 с, мазок 7,58–7,71 с (оба порога не пройдены, ~4–5× медленнее macOS — цена JS/wasm без SIMD). A55 не снят | `pro-sc-02` (чанки `SculptMesh` + локальные нормали) нужен раньше, чем мазок станет интерактивным на 1,2 млн; лимит `pro-sc-09` для веба должен исходить из размера, который проходит этот же бюджет |
...
**Веб-замер, наконец сделанный, а не отложенный.** Настоящий headless
Chrome (`--headless=new`), не предположение:

| Операция | Способ | время |
|---|---|---|
| экспорт 200k (та же решётка 316×316) | `dart compile js` | 353,5 мс |
| экспорт 200k (та же решётка 316×316) | `dart compile wasm` | 427,4 мс |
| импорт 30 МБ (настоящий `.glb`, 625 681 вершина, 1 248 200 треугольников, построен `GltfWriter`, проверен собственным round-trip) | `dart compile js`, `GltfLoader().load()` через `fetch` | ≈10 мс |

Обе эталонные операции `ui-34d`'s собственного условия — на два порядка ниже
секундного порога. `ui-34d` (web worker) не строится: условие
«p0-07/p0-08 показывают заморозку дольше 1 с» не сработало ни на одной из
двух операций.
...
- **Chrome снят тем же днём, тем же `flutter test`.** Причина, по которой
  браузер якобы нуждается в `profile_web.py` и вкладке на переднем плане —
  верная для `p0-02`'s собственного `requestAnimationFrame`-замера — здесь не
  применима: этот тест не просит кадра, значит `flutter test --platform
  chrome` проходит его целиком headless, как и любую другую платформу. Три
  прогона на этой же машине: открытие+BVH — 24,2–24,4 с (порог 3 с не
  пройден), мазок целиком — 7,58–7,71 с (пороги 8/16 мс не пройдены). Chrome
  медленнее нативного прогона примерно в 4–5 раз на той же арифметике — цена
  JS/wasm без SIMD-пути, а не что-то специфичное для скульптинга. **Galaxy A55
  по-прежнему не снят** — нужен телефон в руках, как и у `p0-03`.
