...
**0.2б — дерево для пикинга (`mesh-20`), тот же стенд, дата 2026-09-10.**
`MeshBvh` поверх `TriangleBvh` по треугольникам плана.

| Что | 50k треугольников | 200k треугольников |
|---|---|---|
| `MeshBvh.rebuild` | 12,9 мс (518 нс/грань) | 54,3 мс (544 нс/грань) |
| `MeshBvh.refit` | 1,10 мс (44 нс/грань) | 4,57 мс (46 нс/грань) |
| `MeshBvh.raycast` | 0,4 мкс | 0,5 мкс |

- **Порог `refit = rebuild` из mesh-20 выполнен:** 4,57 мс против 54,3 —
  в двенадцать раз дешевле и укладывается в 5 мс, которые p0-10 назвал
  условием перетаскивания на 200 тыс.
- **Луч — полмикросекунды**, то есть клик по грани бесплатен даже на каждом
  движении мыши. Это подтверждает p0-10 на рабочем коде: пикинг остаётся
  на CPU.
- Перестройка дерева (54,3 мс) сопоставима с перестройкой плана (69,8 мс), и
  обе нужны после топологической правки. Вместе это 124 мс на 200 тыс. — не
  кадр, но и не то, что делают в кадре: `OpResult.topologyChanged` ровно для
  того и есть, чтобы отличать эти правки от перетаскивания.
