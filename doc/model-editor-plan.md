...
| ux-26 | **A message log and mouse hints** (comparison §2.11). A Console panel keeps every `said`, refusal and warning with its author and time, filterable by person and agent; the status line shows what LMB, MMB and RMB do right now under the active scheme and tool; `get_console {since}` for the agent | app + mcp | S | P1 | ux-17, ux-04 | ten commands → ten entries in order, refusals marked; the hints change when a tool is armed; `get_console` returns the same entries |
