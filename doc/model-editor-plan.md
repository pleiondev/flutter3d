...
| view-21-brush-cursor-pressure ⚠ | A ⌀140 cursor as a Flutter overlay or a surface-hugging ribbon via `HitResult.normal`; `pressure` for a stylus only; strokes batched once per frame. ⇢ pro-sc-08, ui-29. Built (partly) with `pro-sc-08`: the ⌀140 cursor is `kSculptCursorDiameter` drawn through the viewport's own `brushRadius` circle — `ux-24` brought that overlay forward for weights, and this is the same circle in the mode it was written for — and `pressure` reaches a stroke's strength through `InputPolicy`, stylus only, with a finger staying on the camera. **What is not built is the per-frame batching:** a sample is its own `SculptStroke` inside the drag's transaction rather than a frame's worth of points collected into one, which is affordable because each command journals only what its own brush touched and because the *upload* is already once per frame (`pro-sc-05`'s `SculptUpload`) — the cost the batching was written against. Left open rather than claimed | app | S | 4 | view-03, 09, 14 | pressure 0.5 → strength 0.5; a finger doesn't create a stroke |
