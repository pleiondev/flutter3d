...
| gal-06 | **The agent inserts from the gallery**: `gallery.list {category?, licence?}` and `gallery.insert {id}`, answering with the ids the insert made — the same `structuredContent` every other tool answers with | mcp + app | S | P2 | gal-03, ux-19 | an agent lists the lighting, inserts a floor lamp and moves it by the id it got back |
