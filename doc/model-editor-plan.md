...
| ux-39 | **Inset, bridge and edge slide** as `flutter3d_mesh` operations and commands with hints and card support — reversing `tools.dart:17-22`'s "deliberately not" for the game-ready minimum: panels and windows, joining pieces, moving loops | mesh + core + app | L | P2 | ux-29 | `validate()` holds after each across 500 random applications over three seeds; an inset of one quad gives five faces; a bridge of two four-edge loops gives four quads; a slid vertex stays on its edge |
