...
| ux-37 | **Workspaces: Essential and Full** (review §7). Essential, the default: Object, Material, Scene; Full adds Mesh and Animation; chosen in settings and in Quick Setup; unready modes never show in either (`ux-07`). Moved up from the review's P2 because Quick Setup asks for it | app | S | P1 | ux-07, ux-09 | an empty settings store shows three modes; switching to Full shows five with no restart; a key for a hidden mode refuses and names the workspace |
