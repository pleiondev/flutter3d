...
| ux-32 | **Surfaces to the handoff** (review §6.2): the status, panel and background tones and the selection colour match the handoff table, or the difference is fixed in `theme_test` as deliberate (`shell.dart:166, 188`, `theme.dart:150, 250, 283`); Roboto is set in the theme | app | S | P2 | ui-38d | `theme_test` holds every tone against the table or an explicit named exception |
