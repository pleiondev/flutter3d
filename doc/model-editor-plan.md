...
| gal-03 | **The gallery screen**: a grid of cards by category, a search, a licence chip on every card, and an insert that goes through `ImportInto` so it is one undo step and lands beside what is already open rather than replacing it | app | M | P2 | gal-01, gal-02, ux-06 | inserting from the gallery is one undo; the document that was open is still open |
