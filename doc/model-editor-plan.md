...
| mat-04 ⚠ | Панель «Материал» полная: список материалов объекта, свойства из `builtInMaterialHints` через `HintRow` в токенах M3, «Дополнительно», неактивные ползунки по `LightingModel`; drag — транзакция. Фаза 1 получает урезанную версию mat-04a-n. Сделано: alpha (mode + cutoff), emissive (цвет + strength), все пять слотов текстур (`SetTexture`/`_fieldSet` уже несли всю нужную машинерию, правки не потребовалось), «Дополнительно» (emissive strength, normal scale и occlusion strength — только когда своя карта уже привязана — и double-sided). Не сделано: сам пикер шейдера — у `SurfaceMaterial` нет поля для того, какую из шести `LightingModel` материал просит, только бит `unlit`; завести его — правка схемы, которая касается каждого читателя и писателя (glTF, OBJ, `.f3d`, MCP-инструменты, привязка рендерера), отдельная строка, а не побочный эффект этой панели | app | M | 2 | mat-01, mat-03, mat-04a-n | у lambert металличность неактивна; hex `#5FD4E4` → (0.373, 0.831, 0.894); три события — один шаг |
...
| anim-07 ⚠ | Экран 07 собран: `AnimationPanel` (`apps/flutter3d_modeler/lib/src/ui/animation_panel.dart`) компонует `ActionsList`, `TimelinePanel`, `SkeletonTree` и `ConstraintsList` в один `ModelerMode.animation`; выбор клипа/дорожки/ключа/сустава и позиция плейхеда — локальное состояние панели, а не `ModelHistory`; наружу идут только `MoveKeys` и `AddClip`. Поглощает таймлайн из ui-28. Не построено: живой предпросмотр позы — `AnimationPlayer` на инстансе нигде в приложении ещё не читает семплированный клип обратно в узлы сцены (оверлей `anim-08` рисует только rest pose), а это отдельная задача — завести плеер на вьюпорт, а не только на панель | app | L | 3 | anim-04, 06, 08 | перетаскивание ромба — `MoveKeys`; кадр `modeler-timeline` |
...
пункта по месту, где она была принята.
...
узкое место — полный пересчёт нормалей на весь меш). Числа сняты только на
macOS; Chrome и Galaxy A55 остались неизмеренными — `pro-sc-01` помечен в
...
решение о числе для веба ждёт замера на двух оставшихся платформах. Это тот же
