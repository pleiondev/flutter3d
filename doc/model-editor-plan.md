...
| ux-31 | **Labels instead of keys** (L11, L20, L21). The Add menu shows names and icons rather than `box`/`plane` (`top_bar_actions.dart:101-106`); Wire draws `EditMesh` edges rather than GPU triangles (`display_modes.dart`); morphs get their own section header instead of sitting under VIEW | app | S | P2 | — | the Add menu has no lowercase key; a quad cube in Wire shows 12 edges; the morph message sits under MORPHS |
