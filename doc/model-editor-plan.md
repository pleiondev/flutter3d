...
| gal-04 | **Sources outside this repository**: Smithsonian Open Access and the Khronos sample assets (both CC0), Poly Pizza and Sketchfab (mixed, mostly CC-BY). Fetched on demand and cached on disk; the filter shows CC0 by default and says plainly what turning it off means. A source that is unreachable is a row that says so, never an empty grid | app | L | P2 | gal-01 | the web build lists and inserts; a source that times out leaves the rest of the grid usable |
