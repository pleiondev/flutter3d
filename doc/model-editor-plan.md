...
| ux-45 | **The person stays in charge of the agent** (review §5.2). "Pause agent" refuses agent calls until resumed; "Undo all agent steps" is one action; a step's author is the client, not one `StepAuthor.agent` for everyone (`history.dart:48`); a person's `amend` of an agent's step makes the step theirs (`history.dart:408`) | app + core | S | P1 | ux-05 | while paused an agent call refuses naming the pause; two clients' steps show two authors; undo-all removes only agent steps, in order |
