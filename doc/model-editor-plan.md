...
| pro-rt-07 | Screen 10: a 290-wide two-block panel, progress in the button's place Built: `BakePanel` at `kBakePanelWidth`. **Two blocks because they are two decisions** — how many quads a retopology comes out at, and which maps are baked onto it once it exists — done once and many times respectively, and a panel that mixed them would put the expensive button beside the one pressed every few minutes. **The progress takes the button’s own place rather than appearing beside it:** while a bake runs, the button that starts it is exactly the wrong thing to leave pressable, since a second bake writes the same image. A Cancel sits under the bar, and the refusal is said once above both blocks rather than as two identical tooltips (`pro_panels_test.dart`) **The panel is built and the mode that hosts it is not, and the row stays partial for exactly that:** `ModelerMode` has no entry for this screen yet (`sculpt` is the one phase-four mode turned on, by `pro-sc-08`), so what is here is the widget, its behaviour and its tests, ready for the mode switch to reach. | app | S | 4 | pro-rt-06, pro-rt-03 | a Job swaps the button for an indicator; a cancel |
...
| pro-sim-06 | Screen 11: type chips, parameters, interactions, a 150-tall bar with a transport and the cache Built: `SimulationPanel` and `SimulationBar`. A chip per kind rather than a dropdown, because the parameters change wholesale with the choice and a dropdown would put the name of the thing being configured somewhere other than above the things configuring it. **Pinning is a selection, not a list of numbers** — the row’s own "pinning through phase-1 selection": the panel says how many vertices hold the cloth up and offers to pin whatever is selected now, because nobody knows a vertex by its number. The strip is under the viewport where the animation mode puts its own, and it draws how much of the cache exists as the bar it is, since a simulation half baked is a scrub bar that runs out halfway and saying so first is cheaper than explaining it after. **The row said 150-wide and means 150-tall** — it is a strip under the picture, not a column beside it (`pro_panels_test.dart`) **The panel is built and the mode that hosts it is not, and the row stays partial for exactly that:** `ModelerMode` has no entry for this screen yet (`sculpt` is the one phase-four mode turned on, by `pro-sc-08`), so what is here is the widget, its behaviour and its tests, ready for the mode switch to reach. | app | M | 4 | pro-sim-04, pro-sim-02 | widget tests; pinning through phase-1 selection |
