...
| ux-18 | **One export dialog, one save flow, terms explained** (L12, review §3.1–§3.3). The top bar's Export menu opens the full export screen with the format preselected (`files.dart:687-691` admits the duplicate); formats come from `builtInModelWriters`, so STL and USDZ appear; "selection only" and "apply modifiers"; warnings after the first line are visible (`files.dart:745-748`); "Save without history" becomes a setting, and probe text like `p0-13n` only shows behind a flag; every term in review §3.3 gets a second tooltip sentence or a checkbox subtitle | app | M | P1 | ux-09 | the export screen lists every registered writer; "selection only" writes one node of two; Save writes with no extra dialog; a test requires a description for every tool in `tools.dart` |
