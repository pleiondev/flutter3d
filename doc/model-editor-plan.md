...
| tut-01 *(found writing case 1, 2026-09-15)* | Case 1 found two gaps: (a) `ModelSession.import`/the `import` MCP tool take only a path, with no `ImportOptions` — so an agent cannot choose a unit or an up axis the way the app's own import screen lets a person; case 1's own fixture works around this by building its starting project directly from `fromModelDocument`/`importMeshData` rather than through `session.import`. (b) `readiness.dart`'s `MeshChecks` only run on `EditedGeometry`; a freshly imported, unwelded `ImportedGeometry` object reads "ready to export" even when its mesh has real problems `MeshChecks` would catch once welded. **Decision (this pass, pending owner confirmation): откладываем** — both are real, but neither blocks case 1's own page (it builds the starting project directly, and diagnoses after welding) or any of ui-37d/mat-33d/mat-34d; each gets its own row and pass once cases 4–6 show whether case 6's agent-driven import needs (a) sooner | core + mcp | S | 3 | doc-11a-n, mcp-13n | not scheduled this pass — tracked so case 6 ("an agent beside you") re-checks it before writing that case |
...
| tut-03 *(found writing case 2, 2026-09-15)* | `ModelSession` has no `amend` method — the operation card's own slider calls `_history.amend(to)` directly on the app's own `ModelHistory` (`apps/flutter3d_modeler/lib/src/screen/interactions.dart`), bypassing `ModelSession`/`CommandJournal` entirely. Case 2's own tests confirm `amend` itself works (it re-runs a mesh edit against the document before it, not on top of it), but an agent over MCP has no way to reach it, and a session's own recovery journal never learns the adjustment happened — the file still names the *original* argument on that step after an amend. **Decision (this pass, pending owner confirmation): откладываем** — a real gap for both an agent-driven operation card and crash recovery after a slider drag, but neither blocks any case in this pass (case 2 demonstrates `amend` directly against `ModelHistory`, outside its own committed journal) | mcp + core | S | 3 | mcp-10n, mcp-12n | not scheduled this pass — worth its own row once an agent-facing operation card is asked for |
| tut-04 *(found writing case 2, 2026-09-15)* | The plan's own case 2 asks for "extrude/loop cut/bevel"; `flutter3d_mesh` has real `bevelEdges`/`bevelVertices` functions, but no `ModelCommand` wraps either one — `apps/flutter3d_modeler/lib/src/ui/tools.dart`'s own comment already says "Inset, bevel and merge are" not tools. Case 2 exercises extrude and loop cut only, honestly, rather than inventing a "bevel" command that does not exist at this layer. **Decision (this pass, pending owner confirmation): откладываем** — a real command (`BevelEdges`, presumably) is a normal-sized addition, not a tutorial-content change | core | S | 3 | mesh-33 | not scheduled this pass — the app's own tools panel already knows it has no button for this |
| tut-05 *(found writing case 2, 2026-09-15)* | `Extrude`/`LoopCut`/`TransformElements` all act on "whatever is currently selected", and selecting mesh elements is `ModelSession.select`, not a `ModelCommand` — its own doc comment already names this cost for object-level picking. Case 2 is the first case whose edits depend on it at *mesh*-element level too: its own `.jsonl`, replayed through `CommandJournal.replay` from a cold `ModelProject`, refuses at the first `extrude` with "no faces are selected to extrude" (asserted directly in `tutorial_scenarios_test.dart`). A live session — a person clicking through the app, or an agent calling `select` then `run` over MCP — reaches the case's own fixture without trouble; only a *crash recovery* replaying the file alone cannot. **Decision (this pass, pending owner confirmation): откладываем** — a real recovery-journal gap (`mcp-12n`'s own promise), but case 2's own test suite proves the case *works* live, which is what this pass needs; making mesh-element selection a replayable command is its own row, not a tutorial-content change | mcp + core | M | 3 | mcp-12n | not scheduled this pass — tracked so a future crash-recovery pass (or case 3, if it edits mesh elements too) picks it up |
| tut-06 *(found writing case 2, 2026-09-15)* | Adding a modifier and expecting to see its effect somewhere — the viewport, a headless render, or an export. Nothing reads `ModelObject.modifiers` except the properties panel's own list and `ApplyModifier`, which bakes the stack into the base mesh and removes it: `SceneSync._dataOf` (the live viewport) reads `object.geometry` directly, and neither `render_project.dart` nor `render_sheet.dart` mentions `modifiers` at all. A mirror or an array modifier is invisible everywhere until "Apply" is pressed, which also throws the non-destructive stack away — the opposite of what a modifier is for. Case 2's own page works around this only for its own picture, evaluating the stack by hand (`tool/make_case2_fixtures.dart`'s own `_withModifiersBaked`) rather than through anything a person driving the real app would see. **Decision (this pass, pending owner confirmation): делаем, but not this pass** — this is the most consequential of case 2's findings: a modifier stack with no live preview anywhere is not a smaller version of the feature, it is a different, much less useful one. Wiring `SceneSync`'s own mesh upload through `ModifierEvaluationCache` (already built, `mesh-40`/`mat-18`, just never called from the app) is the shape of the fix | app + core | M | 3 | mesh-40 | scheduled, priority above tut-03/04/05 — the modeler ships a modifier panel today whose own effect nobody driving the real app can see |
| tut-07 *(found writing case 3, 2026-09-15)* | Case 3 ("A lit corner") sets a real point light, a shadow request, a studio environment and bloom through `AddLight`/`SetLightField`/`SetLightTransform`/`SetEnvironment`/`SetSceneLightingField`, then asks for a headless "expected result" picture the way every earlier case gets one. `render_project.dart`'s own `renderProject` builds its `Scene` from two hardcoded directional lights and never reads `ModelProject.lighting` — not the light, not the environment, not the ambient/shadow/exposure/post fields — and `render_sheet.dart`'s `renderSheet` inherits the same blind spot by calling `renderProject` per quadrant. This is the headless-render half of a gap already named at the live-viewport-snapshot layer: `flutter3d_render_job`'s own `scene_from_project.dart` says outright that nothing in the application calls `LightingSync` from the main viewport either (`mat-23`'s still-unfinished wiring). Checked directly rather than assumed: `grep -rn "LightingSync(" apps/flutter3d_modeler/lib/` finds exactly one call site, inside `lighting_sync.dart`'s own `lightOverflowOf` — a throwaway `Scene` built only to count how many lights would overflow the light buffer, never drawn. So nothing anywhere in this build — not the main viewport, not the game-preview route, not either headless render tool — currently draws what a project's own lights, environment, shadow request or post settings actually do to a picture; `LightGizmos` (`mat-25`) draws a light's own marker, which is a different thing. Case 3's own reference picture is real geometry and placement, honestly captioned as not showing its own lighting. **Decision (this pass, pending owner confirmation): откладываем** — wiring `LightingSync` into the main viewport is `mat-23`'s own unfinished integration and a real, normal-sized change; teaching `renderProject`/`renderSheet` to build their scene from `LightingSync` instead of two fixed lights is a natural extension of the same fix, best done together rather than as a tutorial-content patch | core + app + mcp | M | 3 | mat-23, mesh-40 | not scheduled this pass — tracked so whichever pass finishes `mat-23`'s own `LightingSync` wiring updates `renderProject`/`renderSheet` at the same time, and case 3's own page gets a real lit picture then |
| tut-08 *(found writing case 3, 2026-09-15)* | Case 3 asks for a second file merged into a project already open, the way `mat-24`'s own extended scope (2026-09-09 decision, "Import into scene" via `ImportInto`) names for a person clicking through the real app. Checked directly: `grep -rn "\.import(\|ImportInto" apps/flutter3d_modeler/lib/` finds no call site at all — the toolbar's only file action, **Open** (`_openBytesWithImportScreen`'s own `openDocument`), always replaces the whole document, the identical path case 1's own import already uses. `mat-24` is recorded closed via `mat-34d` (2026-09-14), but `mat-34d`'s own text only wires the four Scene-mode panels (`SceneSourcePanel`/`SceneShadowsPanel`/`SceneEnvironmentPanel`/`ScenePostPanel`); it never mentions an import action, and none exists — `mat-24`'s own "Import into scene" half of its extended scope shipped at the command layer only (`import_into_test.dart`, `scene_multi_asset_export_test.dart`), reachable by an agent through `ModelSession.import` today, not by a person in the GUI. Case 3's own scenario drives `importInto` directly against the session for exactly this reason. **Decision (this pass, pending owner confirmation): делаем, but not this pass** — without this, a person cannot perform case 3's own first step in the real app at all, which is a materially different situation from cases 1 and 2 (both fully walkable by hand once written); wiring a real "Import" action onto the existing `ModelSession.import`/`ImportInto` plumbing is UI work over an already-tested seam, not new machinery | app | M | 3 | mat-24, mat-34d, doc-11a-n | scheduled — case 3 is not walkable by hand in the real GUI until this lands, unlike cases 1 and 2 |
...
| tut-10 *(found writing case 4, 2026-09-15)* | Case 4 rigs, weight-paints, poses and morphs a character, then asks for a headless "expected result" picture the way every earlier case gets one. `render_project.dart` never mentions "skin" or "skeleton" anywhere in it (confirmed by reading the file directly) — every mesh draws at its raw bind-pose position regardless of `ModelObject.skeletonIndex` or any joint's own transform, and it never mentions `shapeSet` either, so a shape key's own weight never reaches the picture. So case 4's own reference render shows real geometry, real materials and the file's own import, but neither the bent-elbow pose `PoseJoint` keyed nor the puffed-chest morph `AddShapeDriver`/`KeyShape` set up. This is the same shape gap as `tut-07` (a project's own real state not reaching a headless picture), now for skinning and morphs rather than lighting. **Decision (this pass, pending owner confirmation): откладываем** — a real fix (posing `renderProject`'s own mesh upload through the same skin/shape blend the engine's own vertex shader runs) is a normal-sized addition to the render path shared by two MCP tools, not a tutorial-content change; grouping it with `tut-07`'s own `LightingSync` wiring (both are "`renderProject` draws less of the document than the document holds") is worth doing together | core | M | 3 | tut-07 | not scheduled this pass — tracked alongside `tut-07` so whichever pass teaches `renderProject`/`renderSheet` to read more of `ModelProject` does lighting, skinning and shape blending together rather than three separate passes |
| tut-11 *(found writing case 4, 2026-09-15)* | Case 4's own weight-paint step wants to show the weight-gradient shading S4 built (`weight_gradient.dart`'s five colour stops over an unlit material) in a headless picture, the way `render`'s own `mode` argument already offers `material`/`normals`/`selection`. `RenderShading` (`render_project.dart`) has exactly two members, `material` and `normals` — confirmed by reading the class directly, and `render_tool.dart`'s own `renderModes` list agrees. The real gradient shading is a windowed-engine concept (`SurfaceShading`, `apps/flutter3d_modeler/lib/src/ui/display_modes.dart`) that `flutter3d_model_core` — a plain Dart package with no dependency on the engine above it — has no path to reach, the identical boundary `ShapeDriver`'s own doc comment already draws for why a live driver stays app-layer work. **Decision (this pass, pending owner confirmation): откладываем** — a `weights` render mode is a real, boundable addition (the vertex-colour math itself is small; the boundary this row exists to name is a project structure one, not a hard one), but it is new function, not tutorial content, and no case in this pass blocks on it | core + mcp | S | 3 | anim-11 | not scheduled this pass — worth its own row once a case (or an agent) actually needs a headless weight-paint picture rather than a live one |
| tut-12 *(found writing case 5, 2026-09-15)* | Case 5 retargets `RiggedFigure.glb`'s own walk cycle onto case 4's character with `lockFeet: true` (`RetargetClipJobRequest`'s own default, screen 14's own "Lock feet" toggle). This exact pair of rigs throws a `RangeError` instead of correcting anything — confirmed directly: `tutorial_scenarios_test.dart`'s own case-5 group calls the same request with `lockFeet: true` and asserts the crash. Root cause, read directly out of `flutter3d_rig`'s own `retarget.dart`: `RiggedFigure.glb`'s own clip animates every joint's translation, rotation *and* scale, not only the root's (the doc comment `retargetClip` inherited assumed only a root/hips track ever carries translation); `_lockFeet`'s own `tracksByNodeId` is a plain `Map<int, RigTrack>`, one entry per target joint, so building it for a joint now carrying three retargeted tracks keeps only the last one built — the scale track — silently dropping the rotation track the foot-lock math actually reads; the per-key indexing that follows then treats that three-floats-per-key buffer as four-floats-per-key quaternions and runs past its own end. **Decision (this pass, pending owner confirmation): делаем, but not this pass** — a real, reproducible crash on a real-world fixture, more consequential than a missing feature: `tracksByNodeId` needs to keep every path a joint carries (a `Map<int, Map<AnimationPath, RigTrack>>`, or a two-key lookup), not collapse them. Case 5's own scenario retargets with `lockFeet: false` instead — a real, documented knob on the same request, the honest correction for this pair of rigs today, not a stand-in for the fix | core | S | 3 | anim-17 | scheduled, priority above tut-13 — a crash on a real sample asset is worse than a naming-coverage gap, and both live in the same function |
| tut-13 *(found writing case 5, 2026-09-15)* | Case 5's own screen 14 "Auto-map" step (`looseAutoMap`) is asked to map `RiggedFigure.glb`'s own nineteen joints onto `RigTemplate.humanoid`'s seventeen. It maps nothing at all — confirmed directly by calling `looseAutoMap` on the file's own joint names and reading back an empty `BoneMap`. `looseAutoMap`'s own two rig-family conventions (Mixamo's `mixamorig:LeftUpLeg`, 3ds Max Biped's `Bip01_L_Thigh`) both put the side marker at the very start of a bone's own name once its prefix is stripped; `RiggedFigure.glb`'s own names (`leg_joint_R_1`, `arm_joint_L_2`) put the side in the *middle*, a shape `_looseSide` was never written to read. Case 5's own scenario enters the seventeen-row correction by hand instead — the exact "correction" a person facing the same empty result in the retarget screen's own bone-map table would type in. **Decision (this pass, pending owner confirmation): откладываем** — a real coverage gap in a real, boundable function (a third "side anywhere in the name" reading, or a Khronos-sample-specific synonym table), but not a defect in the retarget pipeline itself, and the bone-map table's own manual correction already reaches the same result a wider auto-map would | core | S | 3 | anim-17 | not scheduled this pass — tracked alongside `tut-12` since both live in `flutter3d_rig`'s own retarget/bone-map code |
| tut-14 *(found writing case 5, 2026-09-15)* | Case 5's own retarget lands through `ApplyClipResult(clipIndex: null)` — a real `ModelCommand` (`rig_job_commands.dart`): it records to the journal, and undoes/redoes through `ModelHistory` like any other. Its own doc comment says outright it is deliberately outside `modelCommandNames`/`modelCommandFromJson`, "the same as `ReplaceDocument`, for a different reason of its own" — wiring an MCP tool an agent could call it under is later, app-integration work. The practical effect, confirmed directly: `tutorial_scenarios_test.dart`'s own case-5 group replays `case5.jsonl` cold from case 4's own saved project and gets refused on line 1 with "names a command this build does not know" — a *different* shape than cases 2–4's own `tut-05` (a command this build already knows, refused only for want of a selection nothing recorded): here the build cannot even name the command at all, selection or not. A live session — a person clicking through the app, or an agent calling `run` directly once a tool exists — reaches the case's own fixture without trouble; only a *crash recovery* replaying the file alone cannot. **Decision (this pass, pending owner confirmation): откладываем** — the same shape and the same reason as `tut-05`'s own deferral: case 5's own test suite proves the case *works* live, which is what this pass needs; registering `applyClipResult` (and `applyJobResult`'s own sibling, if it is not registered either) for replay is an agent-facing MCP row, not a tutorial-content change | mcp + core | S | 3 | mcp-12n, anim-25 | not scheduled this pass — tracked alongside `tut-05`/`tut-03` as the same "recorded but not replayable" family of gaps |
| tut-15 *(found writing case 6, 2026-09-15)* | Case 6 puts an agent's own `--mcp-port` session and a person's own live edit on one shared `ModelHistory` — `mcp_bootstrap_io.dart`'s own doc comment says outright this binds "over ... the same document a person already has open" — and asks whether a session's own recovery `.jsonl`, replayed cold, still rebuilds the document the way case 1's own journal does. It does not, silently. A person's own edit lands through `ModelHistory.run` directly (`ModelerCubit.run`'s own door, no author named, `StepAuthor.person`), never through `ModelSession.run`/`CommandJournal.record` — so it is never on the session's own journal at all. Confirmed directly: `tutorial_scenarios_test.dart`'s own case-6 group replays `case6.jsonl` cold and gets `replay.ok == true`, no refusal — but the replayed material's own roughness reads `SetMaterialField`'s own default (`0.5`), not the real project's `0.35` a person actually set. A quieter shape than every earlier case's own `tut-05`/`tut-14` (both of which at least refuse outright): a crash-recovery file that looks entirely successful and is silently missing every edit made beside the agent's own session. **Decision (this pass, pending owner confirmation): откладываем** — a real integrity gap in `mcp-12n`'s own recovery promise once a person and an agent genuinely share a document, but this pass's own six cases do not depend on recovering from this exact crash, and a fix (recording every `ModelHistory.run` call regardless of caller, or making the distinction explicit in what `journal` promises) is a `CommandJournal`/`ModelHistory` design change, not a tutorial-content one | mcp + core | M | 3 | mcp-12n, mcp-13n, mcp-10n | not scheduled this pass — tracked alongside `tut-05`/`tut-14` as the same "recovery journal promises more than it keeps" family, now for a genuinely shared document rather than a single session's own unreplayable command |
| tut-16 *(found writing case 6, 2026-09-15)* | Case 6 asks for the design handoff's own screen 26, "Agent session" (`doc/design/modeler-handoff/README-дополнение.md`, row 26): a tool-call feed, a history list with "You"/"Agent" badges, undo restricted to the agent's own steps shown visibly, and a six-view contact sheet under the viewport. Confirmed directly by reading `apps/flutter3d_modeler/lib/src/mcp_ui_tools.dart`: `mcp-16d` built exactly the seven `ui.*` tools it scoped (`setMode`, `setSubmode`, `setTool`, `standardView`, `frameSubject`, `openDialog`, `say`) and nothing else — there is no tool-call feed, no author-badged history list, and no contact sheet anywhere in the app. `--mcp-port` itself and the undo restriction underneath it (`mcp-10n`) are both real and working, confirmed directly by case 6's own test — only the screen the design handoff describes to make either one visible to a person watching is unbuilt, so a person sharing a document with an agent today has no in-app way to see whose step is whose beyond reading the status line's own undo sentence. **Decision (this pass, pending owner confirmation): откладываем** — `mcp-16d` closed on its own stated scope (the seven tools), and screen 26's own panel is a normal-sized UI addition over machinery (`topStepAuthor`, the tool-call answers themselves) that already exists; not needed for any of this pass's own six cases to work or be tested | app | M | 3 | mcp-16d, mcp-10n | not scheduled this pass — worth its own row once a person-and-agent shared session is a feature this build actually ships, not only a tutorial case |
| tut-17 *(found closing out tut-00, 2026-09-15)* | The plan's own known candidate: does a file dropped onto the window ever become the open document? `_handleDroppedFile` (`apps/flutter3d_modeler/lib/src/screen/files.dart`) awaits `_openBytesWithImportScreen` and discards the returned `FileOpened` — confirmed directly by reading the function next to `_openFile`'s own call to the same method a few lines below, which switches on the result and calls `_installOpened`. A person drags a file onto the window, answers the import screen's unit and up-axis questions, and the document on screen never changes — the whole feature is a no-op past the dialog. This is the first of the two latent quirks `ui-37d`'s own text named when the handler moved into `screen/files.dart` verbatim ("`_handleDroppedFile` discards the import result") and promised "its own row" for. **Decision: делаем, but not this pass** — small and mechanical (thread the same switch `_openFile` already runs through the drop path too, most naturally by factoring it into one shared helper both call), but it is an app-behaviour change needing its own regression test and `flutter test` run, not a doc-only close-out commit | app | S | 3 | ui-37d | scheduled — a real, currently silent no-op on the app's own drag-and-drop feature |
| tut-18 *(found closing out tut-00, 2026-09-15)* | The second of `ui-37d`'s own two named latent quirks, found fresh while reading `TransformSession` for the row above: opening a new document should forget every mesh-picking cache keyed by the previous document's object ids, the way `_installOpened` already forgets `_elementPickerCache` for exactly that reason (its own doc comment: "ids start again in the new project and a picker held against the old one could match a version and answer about a mesh that is gone"). `TransformSession._otherPickers` (`Map<int, ({int version, MeshPicker picker})>`) is a second such cache, and nothing clears it — confirmed directly, no call site outside its own lazy-build path touches it. A new document whose object ids and versions collide with a stale entry could snap or gizmo against a mesh that is gone. **Decision: откладываем** — real, but lower severity than `tut-17` (it needs an id/version collision across differently sized documents to bite, not "every drop"), and it belongs with whichever pass next touches `TransformSession` rather than a doc-only close-out commit | app | S | 3 | ui-37d | not scheduled this pass — tracked alongside `tut-17` as `ui-37d`'s own two promised follow-up rows, now both written |
