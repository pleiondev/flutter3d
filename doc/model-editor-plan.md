...
| mcp-02n ⚠ | `flutter3d_cpu` becomes a flat package: `cpu_device.dart` drops `package:flutter/widgets.dart`, `cpu_frame_widget.dart` moves into a wrapper. `encodePng` is already pure and travels for free | cpu | M | 1 | mcp-01n | `dart test` (not `flutter test`) runs `flutter3d_cpu`'s suite; the package is in `flatDartPackages`; `tool/dump_fixture.dart` becomes a script — its own comment today explains why it's a test. Done: `cpu_device.dart` names no Flutter import at all; the widget moved to `flutter3d_app/lib/src/cpu_frame_presenter.dart`; `flutter_test` and the `flutter:` environment constraint are gone from the package's own pubspec entirely, and `cross_backend_test.dart` (mcp-04n) now decodes its golden PNGs with no Flutter SDK behind the call — all 150 tests pass under `flutter test` and the file itself under plain `dart test`. Not done: the package still cannot join `flatDartPackages`, because `shader_names_test.dart` and `conformance_test.dart` — both real, both checking something no other file does — dev-depend on `flutter3d_shaders` and `flutter3d_conformance`, and both of those genuinely need the Flutter SDK. `dart pub get` resolves the whole graph including dev dependencies, so the package is flat in every file that matters and not flat in the one sense this rule mechanically checks |
...
| mcp-06n ⚠ | A `render` tool: views `front/back/left/right/top/bottom/iso`. The PNG is returned as MCP image content, not base64 in text. **Blocked on a real decision, not on code**: `flutter3d_mcp_kit`'s own `PictureAnswer`/`pictureResultOf` already carry a PNG as proper MCP image content (`flutter3d_render_mcp` and `flutter3d_sim_mcp` already return pictures this way), and wiring `renderProject` (mcp-05n) behind a new tool in `flutter3d_model_mcp` is a small, mechanical change on its own. What blocks it: `renderProject`'s own `deviceFactory` needs a *real* device to render anything at runtime, and the only one that exists (`flutter3d_cpu`) carries `flutter3d_conformance` → `flutter3d_shaders` (the Flutter SDK) as a dev dependency — the exact same shape every other backend's own `conformance_test.dart` uses (`flutter3d_webgl`, `flutter3d_webgpu` have the identical edge; it is not a `flutter3d_cpu`-specific oversight to fix). `flutter3d_model_core` (mcp-05n) sidestepped this by taking the device as a parameter; `flutter3d_model_mcp` cannot — a host starting it with `dart run` needs it to actually draw a picture, so it would need a genuine dependency on a working device. That either drops `flutter3d_model_mcp` out of `flatDartPackages` (a real, visible regression to "no Flutter SDK to start this tool," the premise the whole `mcp-0xn` chain opens on), or waits on `flutter3d_conformance` losing its own dependency on `flutter3d_shaders` first — explicitly out of scope per that package's own pubspec ("a separate, larger question... not one this touches"). Raised to the owner rather than picked silently | mcp | S | 1 | mcp-05n, doc-19 | `render` on the doc-21 project gives a non-empty image; on an empty project a refusal names the reason |
