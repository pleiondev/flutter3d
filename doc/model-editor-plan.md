...
| ux-29 | **Extrude and Bevel follow the mouse** (review §4.2). After `E` or Ctrl+B the amount follows the pointer through `ux-11`'s modal, with typed values and Esc as everywhere else, and the card still amends it afterwards (`tool_commands.dart:48-50` guesses 10 % and 5 % of the bounds today); Bevel first needs its own command (`tut-04`) | app + core | M | P1 | ux-11, tut-04 | E, a move, a click → one step with the dragged distance; Esc leaves the mesh unchanged; the card amends the value |
