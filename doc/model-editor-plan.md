...
| ux-21 | **What the web build needs on touch** (review §6.1–§6.2). One-finger orbit comes from `ux-04`; below 1200 px Lathe, Auto-rig and Material Studio open as `Dialog.fullscreen` (`lathe_dialog.dart:150-152`, `autorig_dialog.dart:230-232`, `material_studio_dialog.dart:291-293`); narrow shells show the document name and the unsaved marker; tap targets inside panels are 48 px when `LayoutClass` isn't desktop; the phone FAB gets a tooltip | app | S | P1 | ux-04 | at 800×1200 the three dialogs don't overflow; `androidTapTargetGuideline` passes over a real properties panel; a narrow top bar shows "name •" |
