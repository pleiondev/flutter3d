...
| ux-27 | **Panels that resize and fold** (review §4.2). A splitter on the properties panel with the width remembered; `N` folds the panel and `T` the rail, both also from the palette | app | S | P1 | ux-09 | a 400 px width survives a restart; a folded panel gives its width to the viewport in `shell_test` |
