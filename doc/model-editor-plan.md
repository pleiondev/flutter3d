...
| ux-04 | **Orbit from every input; two navigation schemes in settings** (review §4.1, §6.1; comparison §2.2). "Middle-mouse orbit": MMB orbit, Shift+MMB pan, wheel zoom, Alt+LMB standing in for the middle button, trackpad two-finger orbit with Shift pan and Ctrl or pinch zoom. "Left-drag orbit": LMB drag on empty space orbits unless a drag tool is armed, a box starts from the armed Select tool or `B`, RMB held is free-look with WASD. On touch in both: one finger orbits unless a drag tool is armed, a long press starts a box. The scheme names say what they do and name no other product. All of it stays pure classification in `OrbitGestures`/`InputPolicy`, and `alt` is finally read (`orbit_gestures.dart:370-380`). **Free-look deferred, 2026-09-16, with the rest of the row closed.** "RMB held is free-look with WASD" is not a classification at all — it is a second camera controller: `OrbitController` turns about a target, and looking around from where the camera stands while walking it with the keyboard is a different one beside it, with its own speed, its own clamping and its own handover back to the orbit. Everything else in this row is built and tested per scheme per device; the free-look camera is its own piece of work and gets its own row rather than being half-built inside a gesture classifier | app | M | P0 | ux-09 | `orbit_gestures_test` covers each scheme × mouse, trackpad, touch and pen (the pen still never orbits); live: a MacBook trackpad orbits under both schemes |
...
| ux-11 | **Modal transforms, started one of two ways by setting** (review §4.1–§4.2, comparison §2.3). "Modal on press", the Blender preset's default: G/R/S capture the pointer at once, X/Y/Z and Shift+axis constrain, typed digits set the value, LMB or Enter confirm, RMB or Esc cancel through the transaction. "Arm, then drag", Default's: today's flow, but X/Y/Z while armed are captured instead of falling through to delete (`transform_session.dart:124-134`, `tools.dart:272-278`). In both: a readout "Move · X · 0.35 m" with Shift/Ctrl/Esc hints, an axis line across the scene, rotation by the angle around the pivot rather than `dx * 0.01` (`transform_session.dart:396`), Shift for precision, snap chips in the viewport with the step from settings (`transform_modal.dart:131-133`) | app | M | P1 | ux-03, ux-09 | a key-event test: `G`, `X`, a pointer move → a move along X only and no delete, in both modes; a 90° rotation around a pivot 1 m away lands within 1e-4; the snap step is read from settings |
