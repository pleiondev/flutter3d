...
### ap-12: шаблон и четыре демо на конвейере — платформер, остальные впереди

Начат 2026-09-15, закрыт частично и честно: из «шаблона и четырёх демо»
переведён на конвейер один — платформер. Причина остановиться здесь, а не
продолжать все пять за один проход — ниже, после того, что уже сделано.
...
таких три. Этот пункт переводит на конвейер только тот один, что доведён
до конца; dungeon (10 моделей) и racing (5) — впереди.
...
`assets/models`; `key_model_test.dart` сравнивает платформер (уже
`assets_src/`) и dungeon (пока `assets/`, как есть) по их настоящим,
СЕЙЧАС разным путям, а не по общему предположению, что оба совпадают.
...
**Не сделано, честно.** dungeon (10 моделей: игрок, три монстра, ключ,
пикап, записка, выход, два оружия) и racing (5) — не переведены; template
и strategy не переводить нечего, у них нет настоящих моделей. Текстуры
уровня (BSP-материалы) — вне конвейера в принципе, см. находку выше, не
только для платформера, а для всех игр этой архитектуры. Отдельного
CHANGELOG.md ни у одного приложения в этом репозитории нет и никогда не
было (`publish_to: 'none'`, число записано здесь вместо него) — «записано
в CHANGELOG» из строки плана читается как «записано где-то, честно и
