...
### ap-14: документация — закрыта

Начат и закрыт 2026-09-15. Строка плана буквально: «Страница «Assets» на
сайте (исходники, манифест, семейства, CLI, что делать при ошибке hook);
quickstart без шага шейдеров; README пакетов; CHANGELOG; числа тестов и
пакетов по правилу «says N»».

**Новая страница — `site/content/reference/asset-pipeline.md`.** Не
`site/content/core/assets.md` (тот уже существовал и покрывает рантайм —
декодеры, `.f3d`, загрузку, скининг; страница про другое и не
переименована, а дополнена перекрёстной ссылкой в обе стороны). Новая
страница — про то, что строит движок ap-00–ap-13: `assets_src/`,
`dart run flutter3d_build:init`, манифест (`glob`, последний совпавший
правит), семейства текстур по `--target`, `loadModelAsset`'s debug/release
разница, и отдельная таблица «когда хук падает» — четыре симптома,
каждый уже реально встреченный в этой сессии (`asset_directory_does_not_
exist` до первой сборки, `depend_on_referenced_packages` на `hook/`,
пропавшая модель без подкаталога в `assets:`, `flutter clean` после
свежедобавленной зависимости), а не придуманный заранее список. Собрана
настоящим `npm run build` в `site/` — 32 страницы вместо 31, потому что
сборщик сайта берёт список страниц из явного массива в `tool/build.mjs`,
не сканирует каталог: файл без записи там не публикуется вообще, что и
нашлось при первой сборке (страница отсутствовала в `dist/` целиком, без
единой ошибки).

**Quickstart — только тот шаг шейдеров, что действительно ещё нужен.**
Проверено настоящим `git worktree add --detach` на текущем коммите:
`flutter build macos --debug` для `packages/flutter3d/example` без
ручного `flutter3d_impeller`'s `build_shaders.sh` — собирается, и
`flutter3d_impeller/assets/shaders/flutter3d.shaderbundle` появляется
сам, хуком `ap-06`. Но БЕЗ `packages/flutter3d/example`'s собственного
отдельного `build_shaders.sh` та же сборка падает: «No file or variants
found for asset: assets/shaders/example.f3dshaders» — этот бандл `ap-06`
никогда не покрывал (его же «Не сделано» уже называло это прямо). После
обоих шагов — реальный запуск, скриншот куба с PBR-освещением и
текстурой, не просто «сборка прошла». Quickstart переписан по факту:
шаг для `flutter3d_impeller` убран, шаг для примера остался с пометкой,
что это единственный, а не первый из двух. Игры (`dungeon`/`platformer`/
`racing`) никогда не собирали свой отдельный бандл — только канонический
`flutter3d_impeller`'s — так что для них, буквально, шага шейдеров больше
нет вовсе.

**README пакетов.** `site/content/reference/packages.md` не называл
`flutter3d_build` вообще — ни строки под «## Tools», ни ссылки. Добавлена
запись в стиле остальных: что пакет делает, почему не зависимость
движка, что `AssetLayout`/`familiesForTarget` решают, ссылка на новую
страницу.

**CHANGELOG — там, где эта сессия действительно изменила код, не
формальный проход по всем пакетам.** `flutter3d_build`: сводка ap-03–ap-05,
ap-09, ap-10, ap-13, и честно названный текущий предел (хук не сжимает
текстуры). `flutter3d`: `loadModelAsset` (`ap-11`) — и попутно исправлена
собственная ложь первого пункта («The engine's own code is unchanged» —
уже неверно после `load_model_asset.dart`). `flutter3d_bridge`: то же
самое для «byte for byte 0.5.1's» после правки `FixtureVisuals`/
`ActorVisuals` (`ap-12`). Не тронуты: три игры и `flutter3d_screens` — у
приложений в этом репозитории нет и никогда не было собственного
`CHANGELOG.md` (`publish_to: 'none'`), находка уже названа в `ap-12`.

**Числа тестов и пакетов по правилу «says N».** Уже держались зелёными
на каждом коммите этой сессии — этот пункт ничего отдельно не добавляет
здесь, только подтверждает: `dart run tool/structure.dart` — 32 из 32
после всех правок этого пункта тоже.

**Не сделано, честно.** Ни `flutter3d_bridge`, ни игровые приложения не
получили ссылки на новую страницу из своей собственной документации —
только `quickstart.md` и `core/assets.md`; страница находится через
`reference/packages.md`'s собственную запись для `flutter3d_build`, а не
навигацией сайта в целом (у сайта нет центрального меню — только
перекрёстные ссылки между страницами, тот же способ, каким уже связаны
все остальные `reference/`-страницы). Сайт не опубликован — только
собран локально (`npm run build`) для проверки, что страница
действительно попадает в `dist/`, рендерит mermaid-диаграмму и не рвёт
существующие ссылки; настоящая публикация — вне периметра этого пункта.
