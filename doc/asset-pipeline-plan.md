...
### ap-12: шаблон и четыре демо на конвейере — платформер и dungeon, racing впереди

Начат 2026-09-15, закрыт частично и честно: из «шаблона и четырёх демо»
переведены на конвейер два — платформер и dungeon. Причина остановиться
здесь, а не продолжать все пять за один проход — ниже, после того, что
уже сделано.
...
именам приложений, а не по числу приложений с настоящими моделями:
таких три. Этот пункт переводит на конвейер два из них; racing (5
моделей) — впереди.
...
**Не сделано, честно, теперь.** racing (5 моделей) — впереди, тем же
методом; template и strategy — по-прежнему нечего переводить. Текстуры
уровня — та же названная выше архитектурная граница, теперь дважды
подтверждённая (у dungeon свои BSP-материалы, той же формы, что у
платформера).
