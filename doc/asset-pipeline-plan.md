...
### ap-08: цепочка мипов — закрыт, доведён до реального кадра

Закрыт 2026-09-15. Арифметика (`buildMipChain` в
`packages/flutter3d_formats/lib/src/ktx2/encode/mip_chain.dart` — Kaiser-фильтр
с окном `beta = 4`, гамма-корректная фильтрация sRGB-каналов через декод в
линейное пространство и обратно, ренормализация нормалей на normal map,
сохранение alpha-test coverage через бинарный поиск масштаба по методу
Кастано) уже существовала на дереве и уже была протестирована изолированно
(`test/mip_chain_test.dart`, 6 тестов) — то, чего не было, это склейки с
остальным конвейером и настоящего кадра, который приёмка просит буквально:
«эталонный кадр на CPU-бэкенде с моделью вдали отличается от кадра без мипов
меньше порога шума».

**Склейка — одна функция, а не новый класс.** `writeKtx2WithMips` в новом
файле `mipped_ktx2_writer.dart` строит цепочку через `buildMipChain` и
пишет каждый уровень через `ap-07`'s `writeKtx2` одним вызовом — экспортирована
из `flutter3d_formats.dart` рядом с обеими половинами, которые она соединяет.

**Уровни пишутся несжатыми, и это названная граница, а не недосмотр.**
`encodeBc1`/`encodeBc3`/`encodeEtc2Rgb8`/`encodeAstc4x4` — все четыре
`requireWholeBlocks` (кратность четырём), а цепочка мипов по построению
доходит до 2×2 и 1×1 — ни то, ни другое не целый блок ничего. Настоящий GPU
хранит такой уровень дополненным до целого блока; ничего в этом пакете
такого дополнения не делает, значит `writeKtx2WithMips` пишет
`VkFormat.r8g8b8a8UNorm`/`r8g8b8a8Srgb` — тот же путь без Flutter, которым
`_uploadRgba8` в `texture_upload.dart` уже читает многоуровневую несжатую
цепочку. Протянуть настоящее сжатие по цели через цепочку такой глубины —
работа `ap-09`, не решённая здесь тайно.

**Настоящая находка, отдельная от акта склейки: `buildMipChain` и
`createTextureFromPixels` расходятся в арифметике половинки размера не по
степени двойки.** `buildMipChain` округляет размер каждого уровня ВВЕРХ
(`(current.width / 2).ceil()`), а `CpuDevice.createTextureFromPixels`'s
собственная проверка размера каждого переданного уровня мипов округляет
ВНИЗ (`w > 1 ? w >> 1 : 1`) — для степени двойки (это тестовое дерево:
64×64) оба метода совпадают на каждом шаге, но для произвольного размера
(например, 5×5, тот самый случай, что `mip_chain_test.dart`'s собственный
тест «нечётный размер» уже проверяет на уровне одной функции) они
разойдутся, и `createTextureFromPixels` откажет всей цепочке с `null`, а не
тихо возьмёт часть её. Не исправлено здесь — исправление меняет либо
округление в `buildMipChain`, либо в `createTextureFromPixels`, и то, какая
сторона права, решает `ap-09`/`ap-05`, у которых будет настоящий контент
непроизвольного размера, а не эта сессия, у которой его нет. Названо, а не
скрыто: тест на не-степень-двойки текстуру, прошедшую весь путь до
`createTextureFromPixels`, не написан, потому что написать его — значит либо
подтвердить расхождение (что не чинит его), либо тихо выбрать сторону.

**Настоящий кадр, доказывающий не «цепочка существует», а то, что она
исправляет то, ради чего написана.** Первая попытка сравнить «с цепочкой»
против «без цепочки» через `uploadEncodedImage` дала одинаковый результат в
обоих случаях — не ошибка теста, а настоящая находка о самом движке:
`_uploadKtx2` в `texture_upload.dart` для одноуровневого несжатого RGBA8-файла
уходит в `_uploadRgba8`, которая САМА строит цепочку через `MipChain.build`
(`flutter3d_hardware`), если семплер просит мипы — «без цепочки» с таким
файлом не бывает, движок достраивает свою. `MipChain.build`'s собственный
доккомментарий уже называет то, чем эта наивная цепочка отличается от
`ap-08`'s: «Averaged in linear space… Averaging sRGB bytes as though they
were linear darkens every level… until a distant surface is visibly murkier
than a near one» — то есть в дереве уже был живой, работающий, но
гамма-некорректный способ строить цепочку, и `ap-08`'s настоящая ценность —
не «цепочка появилась», а «эта наивная цепочка ошибалась, а новая — нет».
Тест (`packages/flutter3d_cpu/test/mip_chain_render_test.dart`) рисует один
и тот же одно-текселевый чёрно-белый шахматный узор на маленькой удалённой
плоскости (плоскость 6×6 на z = −60 при fovY = 1 рад — около 4–5 экранных
пикселей при текстуре 64×64, что даёт `footprint` около 14.5 текселей на
пиксель и уровень детализации около 3.9 по семёркой цепочки) дважды: один
раз через `writeKtx2WithMips(..., srgb: true)`, другой — через одноуровневый
`writeKtx2` с тегом `r8g8b8a8UNorm` (не sRGB), что и запускает
`_uploadRgba8`'s собственную наивную цепочку на тех же байтах. Шейдер
(`readSurface` в `cpu_shaders_surface.dart`) декодирует `base_color_texture`
как sRGB безусловно, независимо от тега файла, так что оба рендера проходят
через один и тот же `toLinear()` — разнится только то, что до него дошло.
Средняя яркость патча: **101.4 у гамма-корректной цепочки против 67.2 у
наивной** на тех же исходных байтах — разница почти в 35 единиц на
масштабе 0–255, вчетверо больше порога `+15`, которым закреплён тест, и в
точности то направление, которое `mip_chain_test.dart`'s собственный тест
«small level is near mid-grey, not dark» уже предсказывает на уровне одной
функции. Три теста в `mipped_ktx2_writer_test.dart` (полный круговой проход
через `Ktx2Texture.parse`, sRGB-путь, ренормализация нормалей после
круглого рейса) и один интеграционный в `mip_chain_render_test.dart` — все
зелёные, `dart test` в `flutter3d_formats` — 317 тестов без Flutter SDK,
`flutter test` в `flutter3d_cpu` — полный набор без единого нового провала.

Не сделано, честно: сжатые (BC/ETC2/ASTC) уровни ниже целого блока — см.
выше, работа `ap-09`; расхождение округления «вверх» против «вниз» для
не-степени-двойки размеров — названо, не исправлено, решение ждёт
владельца этого или следующего пункта; настоящий телефон/GPU для замера —
тот же класс ограничения, что уже называют `net-00`/`wg-00` в
`doc/tooling-plan.md`, здесь неприменим напрямую (сравнение доказано на
софтверном рендерере, которому реальное железо не нужно по определению
приёмки).
