...
`dart analyze`/`dart test` в `flutter3d_build` — чисто, 76 тестов (было 63,
+13: `init_test.dart`). `dart run tool/structure.dart` — 32 из 32 правил
зелёные, числа тестов в `ARCHITECTURE.md`/`README.md`/`site/content/
quickstart.md`/`site/content/reference/testing.md` обновлены тем же
проходом (7485 → 7498; таблица `flutter3d_build` 63 → 76; сумма строк
7466 → 7479, тот же разрыв в 19 тестов из `packages/*/example/test`, что и
до этого пункта).
