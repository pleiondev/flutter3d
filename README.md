...
| [`packages/flutter3d_session`](packages/flutter3d_session) | The run lifecycle that ties a game, a device and a screen together, and — since `flutter3d_screens` folded in by the package-merge plan — the screens that are not the game: settings, volumes, rebinding, credits |
...
7514 tests across forty-five packages and ten applications, and the only
